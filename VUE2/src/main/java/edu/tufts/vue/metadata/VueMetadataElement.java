...
public class VueMetadataElement implements tufts.vue.XMLUnmarshalListener {
  ...
  // public static final String VUE_ONT = Constants.ONTOLOGY_URL;//+ONT_SEPARATOR;
  // //"vue.tufts.edu/vue.rdfs";
  ...
  static {
    Log.info("ONTOLOGY_NONE " + Util.tags(ONTOLOGY_NONE) + " " + Util.tag(ONTOLOGY_NONE));
  }
  ...
  /**
   * don't know how TAG originally meant to be used, but a bug spent years saving
   * maps with that
  ...
  public static final int ONTO_TYPE = 2; // can barely find any old maps with this type set, lots of code refs that send
                                         // us an OntType instance will cause it to be set tho
  public static final int SEARCH_STATEMENT = 3; // tag used be search code, ends up being alias for OTHER, tho we never
                                                // have these in a MetadataList
  /** access: LWIcon */
  public static final int OTHER = 4; // Current impl code has hardcoded this to mean "merge source meta-data", tho it
                                     // always ended up being turned into TAG
  public static final int RESOURCE_CATEGORY = 5; /// Only ever use for actual LWMap meta-data: e.g., dc:creator, set in
                                                 /// MapInspectorPanel

  static final int VME_EMPTY_IGNORE = Integer.MIN_VALUE; // mark for ignoring in MetadataList.SubsetList.add during
                                                         // deserialize

  static final String[] _Types = { "TAG", "CAT", "ONTO", "SEARCH", "OTHER(Merge)", "ResCat" };

  /**
   * an object to make most objects look okay to MetadataList (except OntoType)
   * and
   * prevent it from resetting type -- we should never see this key/value
   * anywhere.
   */
  ...
  public static VueMetadataElement createSourceTag(String sourceString) {
    ...
    // try leaving object empty: RDFIndex no longer cares, and this shouldnt show up
    // in
    // interactive UI
    return vme;
  }
  ...
  // For castor mapping file hacks -- getters always null to remove from future
  // save files.
  // These names make more sense when reading the mapping file.
  /** for mapping backward compat */
  public String getSetterKey() {
    return null;
  }
  ...
  /** for mapping backward compat */
  public void setSetterKey(String k) {
    setKey(k);
  }

  /** for mapping backward compat */
  public void setSetterValue(String v) {
    setValue(v);
  }

  /** for mapping backward compat */
  public void setSetterType(String t) {
    // This is marked as a type java.lang.String in mapping file so we have a real
    // Object we
    // deal with, which we need in order to return null in the getter to remove from
    // future
    // save files.
    if (t == null || t.length() < 1) {
      if (DEBUG.Enabled)
        Log.debug("empty type in old save file: " + Util.tags(t));
      this.type = 0;
    } else {
  ...
  // Note: setType(type) was always called LAST in the old mapping, which was
  // importat as it has
  // side effects to this.obj / this.key that *depended* on type. As we want type
  // to be 1st, not
  // last, in the mapping / XML output, we handle that here via an
  // XMLUnmarshalListener and
  ...
  public void setKey(final String key) {
    if (DEBUG.PAIN)
      Log.debug(this + "; setKey " + Util.tags(key));
    // performance hack: esp. for deserializing, check for key constants and change
    // them
  ...
  public void setValue(String value) {
    if (DEBUG.PAIN)
      Log.debug(this + "; setValue " + Util.tags(value));
    this.value = value;
  }

  /**
   * warning: this has significant side effects -- it might change this.key and
   * this.obj Possibly
   * changing the key is part of an old patch for bad key names. This used to mean
   * that persistance was
   * important: type had to come last, tho we fixed this. This still means,
   * however, that in using
   * the API for this class, call order makes a difference: setType calls that
   * happened last
  ...
  public void setType(int type) {
    ...
    if (obj == null && (type == CATEGORY || type == RESOURCE_CATEGORY || type == ONTO_TYPE)) {
      try {
        ...
        // String[] pairedValue =
        // {getKey().substring(len,getKey().length()),getValue()};
        // -----------------------------------------------------------------------------
        ...
        if (key != null && key.contains("file") && key.contains("custom.rdfs")) {
          if (key.contains("#")) {
            ...
            // this.key = "http://vue.tufts.edu/custom.rdfs" +
            // key.substring(key.indexOf("#"), key.length());
            this.key = key.substring(key.indexOf("#"), key.length());
  ...
  /**
   * a messy and pain inducing hack this was -- appears to rarely be called
   * directly in VUE source, so elimination shouldn't be too bad
   */
  public void setObject(final Object obj) {
    if (DEBUG.Enabled)
      Log.warn("setObject->: " + this + "; obj=" + Util.color(Util.tags(obj), Util.TERM_GREEN));
  ...
  /**
   * THIS IGNORES TYPE -- leaving as is in case of backward compat, and shouldn't
   * matter as the keys should always vary with type
   */
  @Override
