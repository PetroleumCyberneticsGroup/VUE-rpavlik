 ...
 * RDFIndex.java
 *
 * RDFIndex mainly makes use of Apache Jena for VUE map searching. This RDF
 * approach is overkill for
 * VUE's current search needs, but originally began as an effort to create
 * larger, peristent, multi-map
 * indicies. That effort was not completed. The resulting implementation that
 * uses this
 * class creates a new RDFIndex for every new search (over one map or multiple
 * maps), and then runs
 * the search on that fresh index.
 *
 * We do get one very nice feature for free -- the ability to write out a full
 * .rdf file containing
 * a set of map data, although the current impl below tosses out the the key
 * names from OSID meta-data
 ...
 *
 * If, long-term, we really want to keep using the RDF indexing / searching, we
 * could
 * go ahead and just build the VUE meta-data system itself out of RDF
 * properties.
 *
...
public class RDFIndex extends com.hp.hpl.jena.rdf.model.impl.ModelCom {
  ...
  // public static final boolean AUTO_INDEX=
  // VueResources.getBool("rdf.index.auto"); // never implememnted
  public static final String INDEX_FILE = VueUtil.getDefaultUserFolder() + File.separator
      + VueResources.getString("rdf.index.file");
  public static final String ONT_SEPARATOR = "#";
  public static final char ONT_SEPARATOR_CHAR = '#';
  /**
   * name-space used for all VUE node fields (e.g., label,notes) as well as all
   * "proper" meta-data (not tufts.vue.Resource meta-data)
   */
  public static final String VUE_ONTOLOGY = Constants.ONTOLOGY_URL + ONT_SEPARATOR;

  /**
   * Include actual keyword names from tufts.vue.Resource properties as opposed to
   * just putting all values under keyword contentPropertyOf.
   * These will be put in the a VUE_OSID namespace -- either determined from the
   * osid, or put in a general unknown-osid namespace.
   * Todo: not exactly the right thing to do for pure web/http URLResources or
   * local file Resources.
   */
  private static final boolean INDEX_VUE_RESOURCE_KEYWORDS = DEBUG.TEST;

  // The naming convetion here is that a "namespace" has the # at the end, but a
  // "prefix" does not.
  // E.g., a namespace is ready to just have a label/keyword appended, and a
  // prefix might have further
  ...
  // private static final String VUE_OSID_PREFIX = "http://vue.tufts.edu/osid/";
  // // is adding to the path this way a
  // reasonable RDF convention?
  private static final String VUE_OSID_PREFIX = "";
  ...
  static {
    // Log.info(Util.tags(VueResources.getString("metadata.dublincore.url")) + " <-
    // metadata.dublincore.url"); // Yes, all url's are being munged.
  ...
  /**
   * If true, index slides and slide content. Currently, all slide content is
   * filtered out on the result side anyway.
   */
  ...
  final com.hp.hpl.jena.rdf.model.Property contentPropertyOf = createProperty(VUE_ONTOLOGY,
      Constants.CONTENT_INFO_PROPERTY);
  // final com.hp.hpl.jena.rdf.model.Property hasTag =
  // createProperty(VUE_ONTOLOGY,Constants.TAG); // Unused: shows up as Text

  /**
   * will contain the URI's of everything indexed in the map, unless this is a
   * global index,
  ...
  /**
   * a general default index -- available but unused feature up through summer
   * 2012
   */
  ...
  public RDFIndex() {
    // super(com.hp.hpl.jena.graph.Factory.createDefaultGraph()); // createGraphMem
    // for performance?
    super(com.hp.hpl.jena.graph.Factory.createGraphMem());
  }

  /**
   * transpose URI results to LWComponents using our internal mapping to vue
   * components
   * Note that this will NOT remove duplicates from the input results: that should
  ...
  public void indexMap(LWMap map) {
    // this.index.remove(this.index); // I presume this is a means of just clearing
    // the entire index?
  ...
  // Todo: could change to take a LWComponent as the focal, tho creating and index
  // for the tiny amount
  // of content on a slide is a bit overkill -- only do if easier than engaging
  // our other DataTree
  ...
  public void indexAdd(final LWMap map, final boolean metadataOnly, final boolean searchEverything_IS_IGNORED) {
    if (DEBUG.Enabled && searchEverything_IS_IGNORED)
      Log.debug("Note: \"search-everything\" bit is now ignored.");
    // If we want slide content, change default index to always index everything --
    // will hardly make
    // a difference just adding slides, and they can be (are currently always)
    // optionally filtered out later anyway.

    if (DEBUG.RDF)
      Log.debug("indexAdd: begin; freeMem=: " + Runtime.getRuntime().freeMemory());

    final com.hp.hpl.jena.rdf.model.Resource mapRoot = this.createResource(map.getURI().toString());

    if (DEBUG.RDF)
      Log.debug("index: create resource for map; freeMem=: " + Runtime.getRuntime().freeMemory());
    ...
    try {
      ...
      if (DEBUG.RDF)
        Log.debug("index: added properties for map; freeMem=" + Runtime.getRuntime().freeMemory());

      final Collection<LWComponent> searchSet = map.getAllDescendents();

      // final Collection<LWComponent> searchSet;
      // // We always filter out slide content anyway, so no point in allowing it un
      // index
      ...
      // if (searchEverything) {
      // // E.g., this will search everything incuding Slides, and even the
      // MasterSlide (which is a bug)
      ...
      if (DEBUG.RDF)
        Log.debug("index: after indexing all components; freeMem=" + Runtime.getRuntime().freeMemory());

    } catch (Exception ex) {
      Log.error("index", ex);
    }
    if (DEBUG.RDF)
      Log.debug("index: done -- size=" + this.size());
  }

  public Collection<URI> searchWithSPARQL(final String queryString) {
    // if (DEBUG.SEARCH) Log.debug("SEARCH; substring=" + Util.tags(substring) + "
    // queryString:\n" + Util.tags(queryString));
    if (DEBUG.SEARCH)
      Log.debug("searchWithSPARQL; queryString:\n" + Util.tags(queryString));

    final Collection<URI> resultSet = new ArrayList<>();
    final com.hp.hpl.jena.query.Query query = QueryFactory.create(queryString);

    if (DEBUG.SEARCH)
      Log.debug("QF created " + Util.tag(query)
    ...
    if (DEBUG.SEARCH)
      Log.debug("created QEF " + qe + "; memory=" + Runtime.getRuntime().freeMemory());

    final ResultSet results = qe.execSelect();
    if (DEBUG.SEARCH)
      Log.debug("execSelect returned; memory=" + Runtime.getRuntime().freeMemory());
  ...
  // ignore certain keys, or maybe even values, but I can't see how it ever
  // worked.
  // ACTUALLY, it might have been to ignore jena Resouce id's in the results, as
  // at one
  // time I can see that was a field added to the index, tho it would have
  // eliminated
  ...
  // Log.info("MYSTERIOUS FILTERING HAS OCCURRED on \"keyword\" " +
  // Util.tags(fullKeyword), new Throwable("HERE"));
  // continue;
  // }
  // // if (keywordLocation > slashLocation)
  // // resultSet.add(new URI(qs.getResource("resource").toString()));
  // }
  // //if (DEBUG.SEARCH) Log.debug("getURI: " +
  // Util.tags(qs.getResource("rid").getURI()));
  // resultSet.add(new URI(qs.getResource("rid").getURI()));
  // //resultSet.add(new URI(qs.getResource("resource").toString()));
  // } catch (Throwable t) {
  // Log.warn("handling QuerySolution " + qs, t);
  ...
  /**
   * Note this does a search using *our* simple query object:
   * edu.tufts.vue.rdf.Query, NOT
  ...
  public Collection<URI> search(edu.tufts.vue.rdf.Query sparql_builder) {
    if (DEBUG.SEARCH)
      Log.debug("search; query=" + Util.tags(sparql_builder));
  ...
  // "PREFIX vue: <"+VUE_ONTOLOGY+"> SELECT ?resource ?keyword WHERE { ?resource
  // ?x ?keyword }";

  /**
   * General search: search all values, ignoring keywords (a.k.a: search the
   * values for every and any key)
   */
  public Collection<URI> searchAllValues(String substring) {
    if (DEBUG.SEARCH || DEBUG.RDF)
      Log.debug("searchAllValues:   " + Util.tags(substring));

    // Newlines for diagnostic readability
    // This would be a the same as what we'd get from a single-criteria
    // edu.tufts.vue.rdf.Query that had
    // not specific property it was looking for (just a value), if Query supported
    // that.
    final String genericSubstringQuery = "PREFIX vue: <" + VUE_ONTOLOGY
        + ">\n SELECT ?rid ?key ?val WHERE {\n\t?rid ?key ?val FILTER regex(?val, \"" + substring + "\", \"i\")\n}";

    // Note: the WHERE cause is what assigns local variable names to WHATEVER is in
    // the RDF store
    // tuples -- e.g., we have the Jena Resource URI at position 0, the keyword/Jena
    // Property at position 1, and
    // the value at position 2. I suspect that's how it's always ordered in the jena
    // model.
    // Note that we can order the results in the QuerySolution by differing the
    // order
  ...
  /**
   * Extract relevant data of interest from the given VUE component, loading it
   * into a new
   * jena.rdf.model.Resource, and add that to the RDF property tree as a child of
   * the given
   * mapRootResource
   */
  private void load_VUE_component_to_RDF_index(
    ...
    // We need to be able to look up the RDF URI result later to get back to the
    // LWComponent.
    // Note it might be faster to keep a master index of all LWComponents created by
    // sequential
    // ID (as this is a runtime need only) and then we wouldn't have create this map
    // each time,
        ...
        for (Map.Entry e : res.getProperties().entries()) {
          ...
          if (value != null) {
            ...
            if (INDEX_VUE_RESOURCE_KEYWORDS) {
              ...
              // Using contentPropertyOf means that all resource properties have the same
              // keyword name,
              // and thus we can never do a searchs such as "author=bob" amongst resource
              // properties...
    ...
    for (VueMetadataElement vme : metadata) {
      ...
      if (key == VueTermOntologyNone) { // also check #TAG so we don't have to encode that?
        // This is just an optimization, so checking object identity is okay (should be
        // used that way)
        if (value != null && value.length() > 0) {
          // Optimization: this being a "none" term (no keyword), don't bother with empty
          // values
          addStatement(createStatement(r, _propertyNone, value));
        }
        // todo: add special property for #source as well, or refactor everything so
        // that such relationships are pre-established and need no detecting (e.g.,
        // smarter VME)
      } else if (key == null) {
        ...
        // }
        // Note that we do NOT want to skip empty values, as we have a non-empty key,
        // and a search for
        // a key with an empty value might be a valid search type someday (if we had the
        // UI to support
        // it).

        // todo: kind of waste to create/fetch these jena Property instances constantly:
        // if we keep
        // this RDF indexing, someday we could just go ahead and put the RDF property
        // right in the VME
        // object. (And pre-encode all keys and/or only allow encoded keys in all
        // meta-data
  ...
  // final List<VueMetadataElement> metadata =
  // component.getMetadataList().getMetadata();
  // for (VueMetadataElement vme : metadata) {
  // if (DEBUG.SEARCH && DEBUG.RDF) Log.debug("scan " + vme);
  // if (true || vme.getObject() != null) {
  // // BAD SEMANTICS: we check "getObject" then just go ahead and check key &
  // value?
  // final String key = vme.getKey();
  // final String strValue = vme.getValue();
  // if (vme.getKey() == VueTermOntologyNone) { // also check #TAG so we don't
  // have to encode that?
  // // This is just an optimization, so checking object identity is okay (should
  // be used that way)
  // if (strValue != null && strValue.length() > 0) {
  // // Optimization: this being a "none" term (no keyword), don't bother with
  // empty values
  // addStatement(createStatement(r, _propertyNone, strValue));
  // }
  // } else {
  // // todo: kind of waste to create/fetch these constantly: if we keep this RDF
  // indexing,
  // // someday we could just go ahead and put the RDF property right in the VME
  // object.
  ...
  // // Note that we do NOT want to skip empty values, as we have a non-empty key,
  // and a
  // // search for a key with an empty value might be a valid search type someday
  // (if we had
  ...
  // }
  // } else {
  // Log.warn(r + ": null object: " + vme); // used to warn "no statement" -- why
  // ignore key/value if no object in VME?
  // }
  // //statement =
  // this.createStatement(r,createPropertyFromKey(element.getKey()),element.getObject().toString());
  // //addStatement(statement);
  // }

  public void addStatement(com.hp.hpl.jena.rdf.model.Statement statement) {
    if (size() < MAX_SIZE) {
      if (DEBUG.SEARCH && DEBUG.META)
        Log.debug("addStatement: " + statement);
      super.add(statement);
    } else {
  ...
  public static String getUniqueId() {
    ...
    // This might better be called something else, but leaving for now just in case
    // of any
  ...
  private boolean compareStatements(com.hp.hpl.jena.rdf.model.Statement stmt1,
  ...
  @Override
  public com.hp.hpl.jena.rdf.model.Property createProperty(final String nameSpace, final String localName) {
  ...
  // /** Auto-create the property if not already there -- [not possible:
  // super.createProperty already does this] */
  // public com.hp.hpl.jena.rdf.model.Property findProperty(final String
  // nameSpace, final String localName) {
  // final com.hp.hpl.jena.rdf.model.Property property =
  // super.getProperty(nameSpace, localName);
  ...
  /**
   * Get/create a property. If the key isn't domain/ns qualified, default to the
   * VUE_ONTOLOGY name space
   */
  public com.hp.hpl.jena.rdf.model.Property getPropertyFromKey(String key) {
    // if (DEBUG.SEARCH && DEBUG.RDF) Log.debug("createPropertyFromKey " +
    // Util.tags(key));
    ...
    if (words.length == 1) {
      ...
      // Note this means anything after a *second* '#' in the string will be entirely
      // ignored
  ...
  /**
   * Note that while VUE.java currently has code to call getDefaultIdex, it's
   * never used.
  ...
  /**
   * E.g., lets say a key has the name "Clubhouse Name" -- this will turn it into
   * "Clubhouse+Name"
   * or "Total Time Online (mm)" to "Total+Time+Online+%28mm%29"
   *
   * NOTE: tho in the second case, jena will let us create a Property at runtime
   * with that name,
   * but if we attempt to export it, we get
   * com.hp.hpl.jena.shared.InvalidPropertyURIException at
   * com.hp.hpl.jena.xmloutput.impl.BaseXMLWriter.splitTag(BaseXMLWriter.java:345),
   * so apparently
   * URLencoder isn't a full-proof encoding strategy. Oh, and also,
   * "Clubhouse+Name" is converted
   * to prefix declaration that includes "...#Clubhouse+", and then just "Name" is
   * used when
    ...
    try {
      ...
      if (ontEnd != -1) {
        ...
        if (name != encodedName) {
          ...
          if (DEBUG.RDF)
            Log.debug("encoded " + Util.tags(key) + " to " + Util.tags(encodedKey));
        }
      } else {
        encodedKey = java.net.URLEncoder.encode(key, "UTF-8");
        if (DEBUG.RDF && encodedKey != key)
          Log.debug("encoded " + Util.tags(key) + " to " + Util.tags(encodedKey));
      }
