...
public class ActionUtil {
  ...
  private final static URL XML_MAPPING_DEFAULT = VueResources
      .getURL("mapping.lw.version_" + XML_MAPPING_CURRENT_VERSION_ID);
  ...
  private final static String VUE_COMMENT_START = "<!--";
  private final static String OUTPUT_ENCODING = "US-ASCII";
  private final static String DEFAULT_WINDOWS_ENCODING = "windows-1252"; // (a.k.a Cp1252) for reading pre ASCII
                                                                         // enforced save files from Windows
  private final static String DEFAULT_MAC_ENCODING = "UTF-8"; // "MacRoman" not supported on Windows platform

  /**
   * Our default is to always read with an input encoding of UTF-8, even if the
   * XML
   * was written with a US-ASCII encoding. This is because pure ascii will
   * translate
   * fine through UTF-8, but in case it winds up being that the XML was written
   * out my
   * the marshaller with a UTF-8 encoding, we're covered. (tho maybe with
   * extremely
   * save files with platform specific encodings, (e.g, MacRoman or
   * windows-1255/Cp1255) we'll lose a special char here or there, such as
   * left-quote
   * and right-quote).
   */
  private final static String DEFAULT_INPUT_ENCODING = "UTF-8"; // safest default input encoding

  // Note: the encoding format of the incoming file will normally either be UTF-8
  // for
  // older VUE save files, or US-ASCII for newer files. In any case, the encoding
  // is
  // indicated in the <?xml> tag at the top of the file, and castor handles
  // adjusting
  // for it. If we want to write UTF-8 files, we have to be sure the stream that's
  // created to write the file is created with the same encoding, or we sometimes
  // get
  // problems, depending on the platform. We always READ (unmarshall) via a UTF-8
  // stream, no matter what, as US-ASCII will pass through a UTF-8 stream
  // untouched,
  ...
  /**
   * A static method which displays a file chooser for the user to choose which
   * file to save into.
  ...
  public static File selectFile(String title, final String fileType) {
    ...
    if (fileType != null && !fileType.equals("export"))
      ...
      // VueFileFilter(VueFileFilter.ZIP_DESCRIPTION));
    } else {
      ...
      // SIMILE chooser.addChoosableFileFilter(new
      // VueFileFilter(VueFileFilter.SIMILE_DESCRIPTION));
      saveChooser.addChoosableFileFilter(new VueFileFilter(VueFileFilter.IMAGEMAP_DESCRIPTION));
    ...
    adjustExtension();
    int option = saveChooser.showSaveDialog(VUE.getDialogParentAsFrame());// ,
    ...
    if (option == VueFileChooser.APPROVE_OPTION) {
      ...
      /**
       * 2009-10-16 There was a bug in here that sliced up the filename, I think
       * removing this block
       * of code should fix the issue. If you had a name like Object.Generic-Tufts,
       * VUE would get
       * all confused and put files in the wrong directory or make a file called
       * Object the filename
   ...
   * file to open.
   * It returns the selected file or null if the process didn't complete
   * TODO BUG: do not allow more than one dialog open at a time -- two "Ctrl-O" in
   * quick succession
    ...
    if (option == VueFileChooser.APPROVE_OPTION) {
      ...
      if (file.exists()) {
        ...
        if (dir.exists() && dir.isDirectory()) {
          ...
          tufts.vue.VueUtil.alert(chooser, VueResources.getString("actionutil.filenotfound.error") + " " + file,
   ...
   * unmarshall listener, which is required for the proper restoration of VUE
   * objects.
   */
  public static Unmarshaller getDefaultUnmarshaller(Mapping mapping, String sourceName)
    ...
    // as we still have different mapping files for old versions of the VUE save
    // file)
    ...
    // unmarshaller.setWhitespacePreserve(true); // doesn't affect elements! (e.g.
    // <notes> foo bar </notes>)
    // HOWEVER: castor 0.9.7 now automatically encodes/decodes white space for
    // attributes...
  ...
  private static Mapping getMapping(URL mappingSource) {
    if (DEBUG.IO)
      Log.debug("Fetching mapping: " + mappingSource);
  ...
  // throws java.io.IOException //, org.exolab.castor.mapping.MappingException
  {
    ...
    if (DEBUG.IO || DEBUG.INIT)
      Log.debug("Loading mapping " + mappingSource + "...");
    try {
    ...
    if (DEBUG.IO || DEBUG.INIT)
      Log.debug("Loaded mapping " + mappingSource);
    ...
    public boolean preMarshal(Object o) {
      ...
      try {
        ...
        if (key != null && tufts.vue.Resource.isRuntimePropertyKey(key)) {
          if (DEBUG.XML)
            Log.debug(" no-marshal " + Util.tags(o));
          return false;
        } else {
          if (DEBUG.XML)
            Log.debug("marshalling " + Util.tags(o));
          return true;
        }
      } catch (Throwable t) {
  ...
  public static boolean isCastorCompatible(LWMap map) {
    try {
      ...
      tufts.vue.VueUtil.alert(VueResources.getString("actionutil.filesave.error") + t,
  ...
  /**
   * A static method which creates an appropriate marshaller and marshal the
   * active map
   */
  ...
  public static void marshallMap(File targetFile, LWMap map) {
    ...
    try {
      ...
      if (DEBUG.IO)
        Log.debug("created new tmp file " + tmpFile);
    ...
    if (targetFile.exists()) {
      ...
      try {
        ...
        // if (DEBUG.IO) Log.debug(String.format("creating backup named [%s]",
        // backupName));
  ...
  private static void doMarshallMap(final File targetFile, final File tmpFile, final LWMap map)
    ...
    if (OUTPUT_ENCODING.equals("UTF-8") || OUTPUT_ENCODING.equals("UTF8")) {
      ...
      // (MarshallException's can find file?)
      // if (FD == null)
  ...
  /**
   * @param targetFile - if null, map state is untouched, otherwise, map state is
   *                   updated
   */
  private static void marshallMapToWriter(final Writer writer,
    ...
    writer.write(VUE_COMMENT_START
        + " Platform " + VUE.getSystemProperty("os.name")
    ...
    // marshal as document (default): make sure we add at top: <?xml version="1.0"
    // encoding="<encoding>"?>
    ...
    // marshaller.setDoctype("foo", "bar"); // not in 0.9.4.3, must wait till we can
    // run 0.9.5.3+
    ...
    // childList of class:
    // tufts.vue.LWMap: The object associated with IDREF "LWNode[2 "New Node"
    // +415,+24 69x22]" of type
    // class tufts.vue.LWNode has no ID!;
    // - location of error: XPATH: /LW-MAP
    // The object associated with IDREF "LWNode[2 "New Node" +415,+24 69x22]" of
    // type class tufts.vue.LWNode has no ID!
    //
    ...
    // Make modifications to the map at the last minute, so any prior exceptions
    ...
    try {
      ...
      // It will still completely blow away the user's map if there is any kind of
      // error.
      ...
      // VueUtil.alert("The map contains characters that are not supported. Reverting
      // to earlier saved version",
      // -----------------------------------------------------------------------------
  ...
  private static MapReader getMapReaderForURL(URL url, String charsetEncoding, boolean allowRedirects)
    ...
    try {
      ...
      if (file != null) {
        ...
        if (allowRedirects) {
          final URL redirectURL = tufts.vue.UrlAuthentication.getRedirectedUrl(url, 10); // number of redirects to
                                                                                         // follow
          url = redirectURL;
    ...
    // urlStream = new BufferedInputStream(new FileInputStream(file)); // remove
    // file:/ from the begining of the file

    // // urlStream = url.openStream();
    // } else {
    // redirectedUrl = tufts.vue.UrlAuthentication.getRedirectedUrl(url,10); //
    ...
    // InputStreamReader(urlStream, charsetEncoding));

    // if ("file".equals(url.getProtocol())) {

    // File file = new File(url.getPath());
    // if(url.toString().contains("#")) {
    // file = new File(url.getPath()+"#"+url.getRef()); // special case for dealing
    // with # in filename
    // }
    ...
    // InputStreamReader(tufts.vue.UrlAuthentication.getAuthenticatedStream(url)));
  ...
  /**
   * Input encoding shouldn't matter for the bootstrap reading of the first few
   * lines
   * of the file (as they should be all US_ASCII), tho using
   * DEFAULT_INPUT_ENCODING,
   * instead of null (which will get us the local platform default), would
   * probably
   * make the most sense. We're leaving this as the default platform encoding
   * (null) for now
  ...
  public static LWMap unmarshallMap(java.net.URL url, MapUnmarshalHandler handler)
      throws IOException {
    // We scan for lines at top of file that are comments. If there are NO comment
    // lines, the
    // file is of one of our original save formats that is not versioned, and that
    // may need
    // special processing for the Resource class to Resource interface change over.
    // If there
    // are comments, the version instance of the string "@version(##)" will set the
    // version ID
    // to ##, and we'll use the mapping appropriate for that version of the save
    // file.
    ...
    // We need to skip past the comments to position the reader at the <?xml line
    // for
    // unmarshalling to start. Also, we look at these comments to determine version
    // of the
    // mapping to use, as well as if it's a pre VUE 1.5 (August 2006) save file, in
    // which case
    // we must guess an encoding, and re-open the file using an InputStreamReader
    // with the
    // proper encoding.
    ...
    for (;;) {
      ...
      if (line.startsWith(VUE_COMMENT_START + " Saved")) {
        ...
        // (only for debug), so determining if the save file was written on a Windows
        // box
        // it's not 100% reliable: e.g., if a user somehow had the name "platform
        // Windows",
        // we would mistake this "saved by" for a "saved on", but we're just going to
        // take
        ...
        if (DEBUG.IO)
          Log.debug("Saving version: [" + savingVersion + "]");
      }
      ...
      if ((idx = line.indexOf("@version(")) >= 0) {
        ...
        if (x > 0) {
          ...
          if (DEBUG.CASTOR || DEBUG.IO)
            Log.debug(url + "; Found mapping version ID[" + versionID + "]");
    ...
    if (firstNonCommentLine.startsWith("<?xml")) {
      ...
      if (DEBUG.IO)
        Log.debug("XML head [" + firstNonCommentLine + "]");
  ...
  private static LWMap unmarshallMap(final java.net.URL url,
      ...
      // throws IOException, org.exolab.castor.mapping.MappingException,
      // org.exolab.castor.xml.ValidationException
      throws IOException {
    LWMap map = null;
    ...
    // Reset the reader to the start of the last line read, which should be the
    // <?xml line,
    // which is what castor needs to see at start (it can't handle ignoring
    // comments...)
    reader.reset();
    ...
    try {
      ...
      try {
        ...
        // if (allowOldFormat && me.getMessage().indexOf("Unable to instantiate
        // tufts.vue.Resource") >= 0) {
        // 2007-10-01 SMF: rev forward the special exception to check for once again in
        // new castor version: castor-1.1.2.1-xml.jar
        // TODO: 2009-03-25: upgraded to Castor release 1.3: the below message check may
        // no longer work...
        if (allowOldFormat && me.getMessage() != null
            && me.getMessage().contains("tufts.vue.Resource can no longer be constructed")) {
          Log.warn("ActionUtil.unmarshallMap: " + me);
          Log.warn("Attempting specialized MapResource mapping for old format.");
          // NOTE: delicate recursion here: won't loop as long as we pass in a non-null
          // mapping.
      ...
      // The below three notify calls must be called in exact sequence (file, then
      // version, then completed)
   ...
   * to. XMLObjectFactory will turn it into an LWMap. For the re-mapping to work,
   * it must be a subclass of LWMap.
  ...
  public static final class OLD_MERGE_MAP_STUB extends tufts.vue.LWMap {
    static {
      if (true)
        throw new Error("should-not-classload");
    }
  }
...
/**
 * This class serves as both an UnmarshalListener impl for VUE LWMap's and their
 * components,
 * as well as a handles what to do to / with a map once it's been unmarshalled
 * to get
 * it into it's final, completed state (e.g., in some cases, transformations may
...
class MapUnmarshalHandler implements UnmarshalListener {
  ...
  void notifyVersionOfVueThatSavedMap(String savingVersion) {
    ...
    if (map.getModelVersion() > LWMap.getCurrentModelVersion()) {
      ...
      // // This setFile also sets the label name, so it appears as a modification in
      // the map.
      // // So be sure to do completeXMLRestore last, as it will reset the
      // modification count.
      // if (map.getModelVersion() < 1) {
      // map.setLabel(file.getName());
      // // force save as for old maps as they will no longer work in old stable
      // versions of VUE (1.5 & prior)
      // // if they're saved in this new version of VUE.
      // } else {
      // map.setFile(file);
      // }

      if (DEBUG.DATA && DEBUG.META)
  ...
  void notifyUnmarshallingCompleted() {
    // note that map.setFile should normally have been completed before this is
    // called
  ...
  public void initialized(Object o) {
    if (DEBUG.XML && DEBUG.META)
      Log.debug(" initialized: " + Util.tags(o));
  ...
  public void attributesProcessed(Object o) {
    if (DEBUG.XML && DEBUG.META)
      Log.debug("  attributes: " + Util.tags(o));
  }

  /** @see org.exolab.castor.xml.UnmarshalListener */
  public void unmarshalled(Object o) {
    if (DEBUG.XML && DEBUG.META)
      Log.debug("unmarshalled: " + Util.tags(o));
  ...
  public void fieldAdded(String name, Object parent, Object child) {
    if (DEBUG.XML) {
      ...
      if (DEBUG.META) {
        ...
        // System.out.println("VUL fieldAdded: parent: " + parent.getClass().getName() +
        // "\t" + tos(parent) + "\n"
        // + " new child: " + child.getClass().getName() + " \"" + name + "\" " +
        // tos(child) + "\n");
      } else {
...
final class XMLObjectFactory extends org.exolab.castor.util.DefaultObjectFactory {
  ...
  XMLObjectFactory(Object source) {
    this.source = source;
    if (DEBUG.Enabled)
      Log.debug("new " + Util.tags(this) + "; " + source);
  }
  ...
  public Object createInstance(final Class _type, final Class[] argTypes, final Object[] args)
    ...
    // else if (_type == tufts.vue.MapResource.class || _type ==
    // tufts.vue.CabinetResource.class)
    // type = tufts.vue.URLResource.class;
    else
      type = _type;

    if (_type != type)
      Log.info("request for instance of " + _type.getName() + " yields: " + type);
    ...
    if (DEBUG.Enabled) {
      if ((DEBUG.IO && DEBUG.META) ||
        ...
        // Do not use Util.tags(o) or allow toString to be called on object --
        // unmarshalling can fail
        // if there are side-effects (!!!) due to calling it -- this happened at one
        // point with an
