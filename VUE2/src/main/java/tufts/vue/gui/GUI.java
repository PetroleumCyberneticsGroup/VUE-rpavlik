    ...
    implements tufts.vue.VueConstants/* , java.beans.PropertyChangeListener */
{
  ...
  // public static final Border WidgetBorder = new MatteBorder(WidgetInsets,
  // Color.orange);
  ...
  /**
   * the currently most "active" device as determined by the position of VUE
   * windows
   */
   ...
   * display device will have it's own coordiates in the global space of all
   * attached
   * display devices. Will also record inset margins as termined by
   * java.awt.Toolkit.
   */
  private static Screen GScreen;
  /**
   * the maximum coordinates of the entire space of all devices -- note that
   * this will actually include off-screen regions if the display layout is not
   * perfectly
   * rectangular
   */
  ...
  // private static final Color VueColor = new ColorUIResource(new
  // Color(128,0,0)); // test

  private static boolean SKIP_CUSTOM_LAF = false; // test: don't install our L&F customizations
  private static boolean SKIP_OCEAN_THEME = false; // test: in java 1.5, use default java Metal theme instead of new
                                                   // Ocean theme
  private static boolean FORCE_WINDOWS_LAF = false; // test: on mac, use windows look (java metal), on windows, use
                                                    // native windows L&F
  ...
  public static void init() {
    ...
    // ErrorFace = new GUI.Face(errorFont, Font.PLAIN, fontSize+1, Color.darkGray,
    // SystemColor.control);
    ...
    // Focus manager seems to be causing a lot of problems on windows 7 and 8 we've
    // received numerous reports about windows not being
    // clickable which I've tracked back to having Focus manager enabled going to
    // try without it and see if there are any major
    ...
    // if (!skipCustomLAF) level = org.apache.log4j.Level.DEBUG; // always show for
    // the moment
  ...
  private static void installUIDefaults() {
    ...
    if (DEBUG.Enabled)
      UIManager.put("ScrollBar.width", new Integer(64)); // just to know we've taken effect
  ...
  private static void installMetalTheme() {
    ...
    if (Util.getJavaVersion() >= 1.5f && !SKIP_OCEAN_THEME) {
      ...
      try {
        Theme = (javax.swing.plaf.metal.MetalTheme) Class.forName(className).newInstance();
  ...
  public static boolean isMacAqua() {
    if (initUnderway)
      throw new InitError("isMacAqua");
    return isMacAqua;
  }

  public static boolean isMacBrushedMetal() {
    if (initUnderway)
      throw new InitError("isMacAquaBrushedMetal");
    return isMacAquaBrushedMetal;
  }

  public static boolean isOceanTheme() {
    if (initUnderway)
      throw new InitError("isOceanTheme");
    return isOceanTheme;
  }
  ...
  private static void loadGraphicsInfo() {
    ...
    // Global raw offsets for the screen: in multi-monitor setup, this will be
    // different
    // for each display.
    ...
    // todo: the below should really be on some kind of observer of a GUI.java
    // state:
  ...
  public static void dumpGraphicsDevice(GraphicsDevice d, String key) {
    // System.out.println("\tDevice " + key + ": " + d + " ID=" +
    // Util.tags(d.getIDstring())
  ...
  /**
   * Return the max window bounds for the given window, for screen device it's
  ...
  /**
   * @return the GraphicsDevice currently determined to be the "active" one for
   *         VUE
   */
  ...
  public static void refreshGraphicsInfo() {
    ...
    // loadGraphicsInfo();
    // else
    // GInsets = Toolkit.getDefaultToolkit().getScreenInsets(GConfig); // this may
    // change at any time
    ...
    // // SwingUtilities.convertPointToScreen(contentLoc,
    // VUE.getApplicationFrame().getContentPane());
  ...
  /**
   * center the given window on default physical screen, but never let it go above
   * whatever
  ...
  // //r.width += r.x;
  // //r.x = 0;
  // // MacOSX won't let us override this when we set a frame's state to
  // MAXIMIZED_BOTH,.
  // // although the window can still be manually positioned there (user or
  // setLocation)
  ...
  private static class ImageIconCache {
    ...
    public void put(String key, Object image) {
      if (DEBUG.IO || DEBUG.IMAGE)
        Log.debug(String.format("caching %10s: %s", key, Util.tags(image)));
      if (image == null)
  ...
  public static Image getSystemIconForExtension(String ext, int sizeRequest) {
     ...
     * + Util.tags(image)
     * + Util.TERM_CLEAR);
     * if (DEBUG.IO) Log.debug("caching: " + ext + "@" + sizeRequest + ": " +
     * Util.tags(image));
  ...
  public static DockWindow createDockWindow(String title, boolean asToolbar, boolean showCloseButton) {
    ...
    // can use a hidden parent for the DockWindow, set them to be never focusable,
    // and
    ...
    if ((Util.isUnixPlatform() && Util.getJavaVersion() >= 1.5f) || VUE.isApplet()) {
      ...
      DockWindow dockWindow = new DockWindow(title, DockWindow.getHiddenFrame(), null, asToolbar, showCloseButton);
  ...
  public static Window getFullScreenWindow() {
    if (FullScreenWindow == null) {
      ...
      // FullScreenWindow = new DockWindow("***VUE-FULLSCREEN***",
      // VUE.getRootFrame());
  ...
  /**
   * Find the containing Window of the given Component, and make sure it's
   * displayed
   * on screen.
   */
  public static void makeVisibleOnScreen(Component c) {
    ...
    if (w != null && !w.isShowing()) {
      if (DEBUG.WIDGET) {
        ...
        if (DEBUG.META)
          tufts.Util.printStackTrace("showing containing window " + GUI.name(w));
      }
  ...
  /**
   * Find the given class in the AWT hierarchy, and make sure it's containing
   * Window
   ...
   *
   * @param clazz - a class that must be a subclass of Component (or nothing will
   *              be found)
   */
   ...
   * if (root != null) {
   * if (true||DEBUG.Enabled) out("ACTIVATING WAIT CURSOR: current =  " +
   * oldRootCursor
   * + "\n\t on JRootPane " + root);
  ...
  private static synchronized void activateWaitCursorInAllWindows() {
    ...
    synchronized (CursorMap) {
      if (DEBUG.FOCUS)
        Log.info("ACTIVATE WAIT CURSOR IN ALL WINDOWS");

      // if (!CursorMap.isEmpty()) {
      if (WaitCursorActive) {
        // VUE.Log.error("attempting to activate wait cursor while one is already
        // active");
        if (DEBUG.FOCUS)
          Log.info("FYI, activating wait cursor for all windows while one is already active");
        // return;
      }
  ...
  private static synchronized void clearAllWaitCursors() {
    ...
    synchronized (CursorMap) {
      if (DEBUG.FOCUS)
        Log.info("CLEAR ALL WAIT CURSORS");
      if (!WaitCursorActive) {
        if (DEBUG.FOCUS)
          Log.info("\t(wait cursors already cleared)");
        return;
      }
  ...
  private static void clearWaitCursor(Component c) {
    ...
    if (oldCursor == null || oldCursor == CURSOR_WAIT) {
      ...
      if (DEBUG.FOCUS)
        Log.info("cleared wait cursor on " + name(c) + " to default");
      ...
      if (DEBUG.FOCUS)
        Log.info("cleared wait cursor on " + name(c) + " to old: " + oldCursor);
  ...
  /**
   * In case window is off screen, size it, then set visible on screen, then place
   * it.
   ...
   * @param loc    - the location of the region to keep entirely on the screen --
   *               may be directly modified by this call
   * @param size   - the size of the region
   *
   *               E.g., can be used to keep tool-tips on-screen.
   *
   *               Note: a complete solution would need to pass in which screen we
   *               want to keep the
   *               point on (for multi-monitor setups) -- for now we just assume
   *               the current VUE
   *               "active" screen.
   *
   ...
   * on
   * the PC -- otherwise when a DockWindow has the focus, you can't use all the
   * global
   * short-cut keys, such as the ability to hide or show a DockWindow with a
   * shortcut!
   *
  ...
  /**
   * make sure there's a colon on the end of the string. E.g., some localized
   * resource
   * strings have the colon built in (inappropriately) -- this helps deal with
   * that
   */
  ...
  /**
   * @return a short name for given object, being smart about it if it's a
   *         java.awt.Component
   */
  ...
  public static String eventName(AWTEvent e) {
    ...
    // //+ " " + VueUtil.pad(25, baseClassName(e.getClass().getName() + "@" +
  ...
  public static final int ALL_MODIFIER_KEYS_MASK = java.awt.event.InputEvent.SHIFT_MASK
  ...
  public static boolean isDoubleClick(MouseEvent e) {
    ...
    return e.getClickCount() > 1
        ...
        // && (e.getModifiersEx() & InputEvent.BUTTON1_DOWN_MASK) != 0 // will not
        // detect in MOUSE_RELEASED
        && !isPopupTrigger(e);
  }
  ...
  /**
   * @return true if 1 click, button 2 or 3 pressed, button 1 not already down &
   *         ctrl not down
   */
  ...
  /**
   * This is basically a dynamic enum type with one significant value associated
   * with each enum key (the localized string)
   * Arrays/collections of these are useful with JComboBox's.
   */

  // would be nice to include a persistance string method and tune class for
  ...
  public static class ComboKey {
    ...
    /**
     * the VueResources.getString(keY) lookup value -- often GUI text that has been
     * localized
     */
    public final String localized;
    /**
     * this is a MASTER ordinal amongst ALL sub-types -- for convenience of a
     * numeric runtime constant only
     */
    ...
    public ComboKey(String k, Map keyMap, Map valueMap) {
      ...
      if (keyMap != null && keyMap.put(key, this) != null)
        throw new Error("duplicate key [" + key + "] for dynamic enum type " + Util.quote(getClass().getSimpleName())
            + "; " + Util.tags(this));
      if (valueMap != null)
        valueMap.put(localized, this);
      ordinal = count++; // could use keyMap.size()
      // todo: force all keys lowercase for lookups
      // KeyMaps.put(getClass(), keyMap); // will be called for every instance, but
      // that's okay
    }
    ...
    /**
     * when implementing an All() in subclasses to return all declared instances,
     * throw this error if key-map is empty
     */
    /**/public static class InitError extends Error {
      public InitError(Class clazz) {
        super("no keys yet for: " + clazz.getSimpleName() + "\n\treference instance(s) of " + clazz.getName()
            + " earlier in calling code to force classloading");
      }
    }
    // public static Object[] All(Class clazz) { // is static: must pass in sub-type
    // if (data().KeyMap.isEmpty())
    // throw new Error("fix your init-order with a declaration change: declared
    // instances haven't been referenced yet");
  ...
  // /** result-operations, take II. Needs more ComboKey support for ROP.All() to
  // work... */
  // public static final class ROP extends ComboKey { private ROP(String s) {
  // super(s /*,ROP.class*/); }
  ...
  // // Declaring them in-class ensures a call to All will always report properly,
  // even during init.
  // // This is probably the closest we'll ever get to a DSL in Java... I see why
  // Scala.
  // }
  ...
  // probably move this & LocalData to something like LWDataTransfer with
  // MapViewer.LWTransfer
  ...
  private static class UnicodeIcon extends IconImpl {
    ...
    public void paintIcon(java.awt.Component c, java.awt.Graphics g, int x, int y) {
      ...
      g.setColor(color);
      // Log.debug("drawing " + glyph + " at " + x + "," + y + "; +" + xoff + ",+" +
      // yoff);
  ...
  public static Font getUnicodeGlyphFont(int pointSize) {
    if (Util.isWindowsPlatform())
      return new Font("Lucida Sans Unicode", Font.PLAIN, pointSize + 4); // this is included in default WinXP
                                                                         // installations
    // setFont(new Font("Arial Unicode MS", Font.PLAIN, pointSize+4)); // this is
    // not
  ...
  /**
   * A class for a providing a label of fixed size to display the given unicode
   * character.
   * A font is selected per-platform for ensuring good results.
   * Todo: can we get a font that will handle this on Linux? ("Lucida Grande" is
   * the default).
   * The fixed sizes is useful in the case where you want a changeable icon, such
   * as a
   * right-arrow / down-arrow for an open/close icon, but the glyphs are actually
   * different
   * sizes. This allows you to fix to the size to something that accomodates both
   * (currently
   * you must tune and select that size manually, and use two IconicLabels with
   * the same size).
   */
  public static class IconicLabel extends JLabel implements Icon {
    ...
    public IconicLabel(char iconChar, int pointSize, Color color, int width, int height) {
      ...
      // todo: this may be a problem for Linux: does it have any decent default fonts
      // that
      // include the fancy extended unicode character sets?
      // setFont(new Font("Arial Unicode MS", Font.PLAIN, (int) (((float)pointSize) *
      // 1.3)));
     ...
     * super.addNotify();
     * if (DEBUG.BOXES) System.out.println(name(this) + " prefSize=" +
    ...
    public void paintComponent(Graphics g) {
       ...
       * for the arrows so am drawing them instead, and just not using the characters
       * at all.
       */
    ...
    public Rectangle getBounds() {
      return new Rectangle(getX(), getY(), mSize.width, mSize.height);
    }
  }
  ...
  public static class PopupMenuButton extends JLabel {
    ...
    // static final char chevron = 0xBB; // unicode "right-pointing double angle
    // quotation mark"
  ...
  private static Border getAquaTextBorder() {
    ...
    try {
      ...
      try {
        ...
        // java 6 update as of June 2009 (1.6.0_13) will fail on above, but should find
        // this:
        abc = Class.forName("com.apple.laf.AquaTextFieldBorder");
      }
      AquaTextBorder = (javax.swing.border.Border) abc.newInstance();
      if (DEBUG.Enabled)
        Log.debug("found Mac Aqua text border: " + abc);
    } catch (Throwable t) {
  ...
  // doc.setParagraphAttributes(0, doc.getEndPosition().getOffset(),
  // mAttributeSet, true);
  // }
  ...
  public static String dragName(DragSourceDropEvent e) {
    return VueUtil.pad(20, baseObjectName(e)) + "; drop=" + dropName(e.getDropAction()) + "; success="
        + e.getDropSuccess();
  }
  ...
  // todo: move to DELAYED-init block for GUI called at end of main during caching
  // runs
  ...
  public static boolean dragImagesSupported() {
    ...
    // 2008-04-14: Windows Vista appears capable of drag images (at least Safari on
    // Vista does it),
    // tho don't know if java 6 is wired up to use it yet (first test was not
    // successful).
  ...
  public static void startRecognizedDrag(DragGestureEvent e, tufts.vue.LWComponent c) {
    ...
    e.startDrag(DragSource.DefaultCopyDrop,
        c.getAsImage(0.5, new Dimension(256, 256)), // as of Java 6, Mac default is to scale to max of 128 in any
                                                    // dimension
        // tho above can be changed via apple.awt.dnd.defaultDragImageSize System
        // Property
  ...
  /**
   * This allows any Component to initiate a system drag without having to be a
   * drag gesture recognizer -- it
   * can simply be initiated from a MouseMotionListener.mouseDragged
   *
   * @param image, if non-null, is scaled to fit a 128x128 size and rendered with
   *               a transparency for dragging
   */
  public static void startSystemDrag(Component source, MouseEvent mouseEvent, Image image, Transferable transfer) {
    if (DEBUG.DND)
      out("startSystemDrag: " + transfer);
    ...
    if (image != null) {
      ...
      // todo opt: could just fill the rect below or to right of what we're about to
      // draw
  ...
  private static void startDrag(Component source,
    ...
    trigger
        .startDrag(DragSource.DefaultCopyDrop, // cursor
            ...
            new GUI.DragSourceAdapter()
        // is optional when startDrag from DragGestureEvent, but not
  ...
  private static class DragStub extends DragGestureRecognizer {
    ...
    protected void registerListeners() {
      /* out("DRAGGER REGISTER"); */ }

    protected void unregisterListeners() {
      /* out("DRAGGER UN-REGISTER"); */ }
  }
  ...
  public static class DragSourceAdapter implements DragSourceListener {
    ...
    public void dragOver(DragSourceDragEvent dsde) {
      if (DEBUG.DND & DEBUG.META)
        out("dragOver: " + dragName(dsde) + " " + dragSource(dsde));
    }

    public void dragEnter(DragSourceDragEvent dsde) {
      if (DEBUG.Enabled || DEBUG.DND)
        out("         dragEnter: " + dragName(dsde) + " " + dragSource(dsde));
      tufts.vue.LWTransfer.markAsLocal(true);
    }

    public void dropActionChanged(DragSourceDragEvent dsde) {
      if (DEBUG.DND)
        out(" dropActionChanged: " + dragName(dsde) + " " + dragSource(dsde));
    }

    public void dragExit(DragSourceEvent dse) {
      if (DEBUG.Enabled || DEBUG.DND)
        out("          dragExit: " + dragSource(dse));
      tufts.vue.LWTransfer.markAsLocal(false);
    }

    public void dragDropEnd(DragSourceDropEvent dsde) {
      if (DEBUG.DND)
        out("       dragDropEnd: " + dragName(dsde) + " " + dragSource(dsde));
    }
  }
  ...
  public static class ResourceTransfer implements Transferable {
    ...
    public synchronized Object getTransferData(DataFlavor flavor)
      ...
      if (DEBUG.WORK || (DEBUG.DND && DEBUG.META))
        System.out.println("ResourceTransfer: getTransferData, flavor=" + flavor);
      ...
      if (DataFlavor.stringFlavor.equals(flavor)) {
        ...
        if (o instanceof java.io.File) {
          ...
          // todo: Resource.getReference? Returns URL if there is one, local package
          // content otherwise
      ...
      if (DEBUG.WORK || (DEBUG.DND && DEBUG.META))
        System.out.println("\treturning " + Util.tags(result));
  ...
  public static class MouseWheelRelay implements MouseWheelListener {
    ...
    /**
     * Create's a MouseWheelListener that forwards events to "head", and if the
     * event hasn't
     * been consumed there (@see AWTEvent.consume()), forwards the event on to
     * "tail".
     */
     ...
     *
     * If no JScrollPane is found to intercept events on, the intercepting component
     * will
     * simply be added as a standard MouseWheelListener.
     *
     * @return true if an intercept was installed, false if a standard
     *         MouseWheelListener was installed
     *
     *         BUG: note that the event delivered to the intercepting listener will
     *         still
     *         have its x,y coordinates in the coordinate space of the original
     *         target, so
     *         the interceptor will currently have to check the event source and
     *         adjust for
    ...
    public void mouseWheelMoved(MouseWheelEvent e) {
      ...
      if (DEBUG.FOCUS)
        Log.debug("MW-HEAD->" + GUI.eventName(e) +
            "\n\t" + getClass().getName() + ": INTERCEPTING dispatch: " + Util.tags(head));
      // first, send to the intercept to see if it wants it
      // TODO: can use SwingUtilities.convertMouseEvent to patch coordinate system of
      // intercepted mouse event
      head.mouseWheelMoved(e);

      // if unconsumed by the intercept, send on to the original target we're
      // overriding (usually, a JScrollPane)
      if (!e.isConsumed()) {
        if (DEBUG.FOCUS)
          Log.debug("MW-TAIL->" + GUI.eventName(e) +
  ...
  private static class DefaultMetalTheme extends javax.swing.plaf.metal.DefaultMetalTheme {
    ...
    DefaultMetalTheme(CommonMetalTheme common) {
      this.common = common;
    }
  ...
  static class CommonMetalTheme {
    ...
    CommonMetalTheme() {
      ...
      // Color VueColor = new ColorUIResource(200, 221, 242); // #c8ddf2 from
      // OceanTheme
  ...
  public static class TabbedPaneUI extends javax.swing.plaf.metal.MetalTabbedPaneUI {
    ...
    public static ComponentUI createUI(JComponent x) {
      if (DEBUG.INIT)
        System.out.println("Creating GUI.TabbedPaneUI");
   ...
   *      the original object.
   *
   *      It saves created ButtonGroup in the Box using a JComponent client
   *      property with key ButtonGroup.class.
   */
  ...
  public static java.util.List<AbstractButton> createSegmentButtonsWithStyle(int numButtons, ButtonGroup buttonGrp,
      String style) {
  ...
  /*
   * private static PropertyChangeHandler PropertyChangeHandler = new
   ...
   * }
   * static class PropertyChangeHandler implements
