 ...
 *
 * This class is way too big. A bunch of inner classes could be separated out
 * (e.g., Key, maybe
 * Property, property event change raising (notify), etc). Beyond that, it could
 * use a major separation of
...
// line-wrapped text. Otherwise, in LWNode's, setting label before size set will
// cause
// the size to be set.
...
public class LWComponent
  ...
  public enum ChildKind {
    ...
    /**
     * Include any and all children in the traversable LW hierarchy, such as slides
     * and their
     * children (pathway contents), and actual layer objects -- the only way to make
     * sure you hit
     * every active LWComponent in the runtime related to a particular LWMap (not
     * including the
     * Undo queue). This will return every LWComponent in the model that has an ID
     * (getID).
    ...
    /**
     * include only default, conceptually significant chilren, leaving out items
     * such a slides, layers and pathways
     */
    ...
    // VIRTUAL -- would be *just* what ANY currently adds, and exclude PROPER --
    // currently unsupported
  }

  /**
   * order of result set for getAllDescendents -- not applicable if collection
   * passed in isn't ordered
   */
  public enum Order {
  ...
  /*
   * // need an IntegerPreference and/or an IntegerRangePreference (that
   * ImagePreference could also use)
   * private static final VuePreference SlideIconPref =
   * IntegerPreference.create(edu.tufts.vue.preferences.PreferenceConstants.
   * MAPDISPLAY_CATEGORY,
   * "slideIconSize",
   * "Slide Icon Size",
  ...
  public enum HideCause {
    ...
    , LAYER_EXCLUDED
    /**
     * we're a member of a pathway that hides when the pathway hides, and all
     * pathways we're on are hidden
     */
    ...
    /**
     * we've been hidden because the current pathway is all we we want to see, and
     * we're not on it
     */
    , NOT_ON_CURRENT_PATH(CAUSE_PATHWAY)
    /**
     * we've been hidden due to the collapse of a parent (different from
     * Flag.COLLAPSED, which is for the collapsed parent)
     */
    , COLLAPSED
  ...
  public enum Flag {
    ...
    DELETED,
    /**
     * we've been hidden due to filtering -- note that this isn't a hide-bit in that
     * children may still be visible even when
     * this is set
     */
    FILTERED,
    ...
    SLIDE_STYLE,
    /**
     * was created to serve some internal purpose: not intended to exist on a
     * regular user map
     */
    INTERNAL,
    ...
    /**
     * for subclasses that want to distinguish between a default size and a
     * validated size (e.g., LWImage)
     * "default size" could actually mean any suggested or invalid size before a
     * final definite size
     */
    , UNSIZED
    ...
    // do we want a generalized LOCKED which means
    // fixed,no-delete,no-duplicate?,no-reorder(forward/back),no-link?
  ...
  // Static { for (Hide reason : Hide.values()) { System.out.println(reason + "
  // bit=" + reason.bit); } }

  public static final java.awt.datatransfer.DataFlavor DataFlavor = tufts.vue.gui.GUI.makeDataFlavor(LWComponent.class);
  ...
  protected float x;
  protected float y;
  // TODO: if we want to support some kind of keep-relative alignment for an
  // object
  // (in it's parent), we couldn't just use a special object on a generic x/y
  // value
  // ptr -- we still need ACTUAL x/y values to render, but we could have an
  // xAnchor/yAnchor, which could even be a list of actions to perform every time
  // the
  ...
  // protected int mFilterBits = 0x0; // may need this to get pathway filtering
  // not in conflict with search filtering
  ...
  /**
   * list of all pathway entries that refer to us (one for each time we appear on
  ...
  // need MetaMap (a multi-map) for XML data-sets that can have more than one
  // value per key
  ...
  // TODO PERFORMANCE: change support could be handled generically, and we could
  // at least lazy-create
  ...
  public static final Comparator XSorter = new Comparator<LWComponent>() {
    public int compare(LWComponent c1, LWComponent c2) {
      // we multiply up the result so as not to loose differential precision in the
      // integer result
  ...
  public void copyStyle(LWComponent styleSource, long permittedPropertyBits) {
    if (DEBUG.STYLE || styleSource == null) {
      System.out.println("COPY STYLE of " + Util.tags(styleSource) + " ==>> " + Util.tags(this) + " permitBits="
    ...
    for (Key key : Key.AllKeys)
      ...
      // (permittedPropertyBits & key.bit) != 0)
  ...
  public void copyProperties(LWComponent source, long propertyBits) {
    if (DEBUG.STYLE)
      System.out.println("COPY PROPS of " + source + " ==>> " + this + " bits=" + Long.bitCount(propertyBits));
    for (Key key : Key.AllKeys)
    ...
    for (Map.Entry<String, String> se : cssStyle.getAttributes().entrySet()) {
      ...
      for (Key key : Key.AllKeys) {
        ...
        if (key.cssName.indexOf(";") > 0) {
           ...
           * out("Can't apply CSS Style property to non-slotted key: " + cssName + " -> "
           * + key);
           * } else {
           * try {
           ...
           * } catch (Throwable t) {
           * System.err.println();
           * tufts.Util.printStackTrace(new Throwable(t), "failed to apply CSS key/value "
           * + cssName + "=" + cssValue);
           * }
           * }
           */
  ...
  /**
   * Describes a property on a VUE LWComponent, and provides an info string for
   * creating Undo names,
   * and for diagnostic output. Implies the ability to set/get the value on an
   * LWComponent by some means.
   */
  // todo: consdier moving all the Key/Property code to some kind of superclass to
  // LWComponent -- LWStyle? Vnode? LWKey? LWState?
  // We'd move it elsewhere, but we'd have to export all sorts of stuff to make
  // all thats needed available,
  // as they get everything currently being inner classes.

  // The generic type TSubclass allows the inner-class impl's of getValue &
  // setValue, in subclasses
  // of LWComponent, to use their own type in the first argument to set/getValue,
  // omitting
  ...
  public static class Key<TSubclass extends LWComponent, TValue> {
    ...
    /**
     * A name for a CSS property that can be used to initialize the value for this
     * key
     */
    public final String cssName;
    /**
     * The unique bit for this property key.
     * (Implies a max of 64 keys that can be known as active to our tools -- use a
     * BitSet if need more)
     */
    public final long bit;
    // /** True if this key for a style property -- a property that moves from style
    // holders to LWCopmonents
    ...
    /**
     * Get the supported property bit mask for the given class in the LWComponent
     * inheritance tree
     * This will only return accurate results after all Key's in the codebase have
     * been initialized.
     */
    static long PropertyMaskForClass(Class<? extends LWComponent> clazz) {
      ...
      if (bitsForClass == null) {
        ...
        if (DEBUG.INIT)
          Log.debug(String.format("CACHED PROPERTY BITS for %s: %d", clazz, Long.bitCount(propMaskForClass)));
    ...
    /**
     * @return the currently stored property mask for the given class: only used
     *         during initialization
     *         Will return 0L (no bit set) if the given class is not in the map
     *         (e.g., java.lang.Object)
     *         This is used to disambiguate between properties that apply only to a
     *         particular
     *         LWComponent subclass while we produce the ultimate merged results for
     *         all classes in
     *         the hierarchy.
     */
    ...
    protected Key(String name, String cssName, KeyType keyType) {
      ...
      if (InstanceCount >= Long.SIZE) {
        this.bit = 0;
        tufts.Util
            .printStackTrace(Key.class + ": " + InstanceCount + "th key created -- need to re-implement (try BitSet)");
      } else
        this.bit = 1 << InstanceCount;
      AllKeys.add(this);

      // Note: this only works if the key is in fact declared in the enclosing class
      // to
      ...
      // System.out.println("BITS FOR " + LWImage.class + " " +
      // PropertyMaskForClass(LWImage.class));
    ...
    // private static final Property BAD_SLOT = EmptyStyle.mStrokeColor; // any
    // (different) slot will do

    /**
     * If this isn't overriden to return non-null, getValue & setValue must be
     * overriden to provide the setter/getter impl
     */
    ...
    // The design of the below two methods, setValue and setValueWithContext, is
    // such that either
    // can be overridden depending on the needs of the Key implementation, but never
    // BOTH.
    // Also, the setValue(TSubclass, TValue) method can never be called directly,
    // except by
    // the single call made to it in setValueWithContext. This is so that the
    // context
    // argument can be completely ignored in simple Key implementations, and only
    // made
    // use of where needed.

    /**
     * non slot-based property keys can override this -- only override ONE of the
     * two setValue methods -- note that this
     * particular method should NOT EVER be called directly -- only overridden
     */
    // what we really want here is some kind of access modifier that make this
    // protected for overriding, but private to calling
    ...
    void setValueWithContext(TSubclass c, TValue value, Object context) {
      // if this has NOT been overriden (the usual case), defer to the default
      // non-context setValue, which will defer to slot
    ...
    private Property getSlotSafely(TSubclass c) {
      ...
      // if (slot == NO_SLOT_PROVIDED) tufts.Util.printStackTrace(this + ": no slot
      // provided");
      return slot;
    }
    ...
    void setStringValue(TSubclass c, String stringValue) {
      ...
      if (slot != NO_SLOT_PROVIDED) {
        ...
        TValue v = getValue(c);
        // handle a few special cases for standard java types, even if there's no slot
        // (Property object) to parse the string
        // This won't work if getValue returns null, as we'll have no class object to
    ...
    private void stringSet(TSubclass c, Object v) {
      setValueWithContext(c, (TValue) v, PROPERTY_SET_DEFAULT); // may want a seperate context here, e.g.,
                                                                // PROPERTY_SET_FROM_STRING
    }
    ...
    /**
     * @return true if the value for this Key in LWComponent is equivalent to
     *         otherValue
    ...
    void copyValue(TSubclass source, TSubclass target) {
      if (!source.supportsProperty(this)) {
        if (DEBUG.STYLE && DEBUG.META)
          System.err.println("   COPY-VALUE: " + this + "; source doesn't support this property; " + source);
      } else if (!target.supportsProperty(this)) {
        if (DEBUG.STYLE && DEBUG.META)
          System.err.println("   COPY-VALUE: " + this + "; target doesn't support this property; " + target);
      } else {
        ...
        if (newValue != oldValue && (newValue == null || !newValue.equals(oldValue))) {
          if (DEBUG.STYLE)
            System.out.format("    COPY-VALUE: %s %s%-15s%s %-40s -> %s over (%s)\n",
                ...
                target,
                oldValue);
          setValueWithContext(target, newValue, PROPERTY_SET_DEFAULT); // may want a specific context here, e.g.,
                                                                       // PROPERTY_COPY
        }
    ...
    public String toString() {
      return name;
    } // must == name for now until tool panels handle new key objects (is this true
      // yet?)
   ...
   * define a default
   * set of setters and getters that automatically handle stuff like undo and
   * positing change
   * notifications. It also allows us to easily attach meta-data to the property
   * itself: e.g.,
   * it's locked, it's overriding a parent style value, it's caching some related
   * computed value,
   * etc.
   */
  protected abstract class Property<T> {
    ...
    void set(T newValue) {
      // final Object old = get(); // if "get" actually does anything tho, this is a
      // BAD idea; if needbe, create a "curValue"
      ...
      // RAISE CHANGE EVENT (for observers -- e.g., repaint, UndoManager, editors,
      // etc)
      // maybe: if (alive()) ?
      LWComponent.this.notify(this.key, oldValue);

      // note: if could handle event raising in Key class, we could make this class
      // static.
      // Tho then to bind to a particular property on a particular LWComponent, you'd
      // need a
      // Binding object. Do we actually bind directly to individual Property
      // instance's
      // anywhere? (Answer: seems no -- was just able to make this class protected
      // instead of
      // public) Could we possibly change class hierarchy such that LWComponent.Key is
      // typed, and
      // subclassed for the various types, and then do away with the Property class
      // hierarchy
      // entirely? -- SMF 2012
    }

    /**
     * This JUST changes the stored value: no notifications of any kind will be
     * triggered, no undo recorded.
     */
    void take(T o) {
      this.value = o;
      if (DEBUG.TOOL)
    ...
    /**
     * impl's can override this to do something after the value has changed (after
     * take() has been called),
    ...
    void setBy(String fromValue) {
      // Could get rid all of the setBy's (and then mayve even all the StyleProp
      // subclasses!!)
      // If we just had mapper class that took a type, a value, and returned a string
      // (e.g., Font.class, Object value)
    ...
    /*
     * void setByUser(Object newValue) { // for tools. Actually, tools using generic
     * setProperty right now...
  ...
  public class EnumProperty<T extends Enum> extends Property<T> {
    EnumProperty(Key key, T defaultValue) {
      ...
      // Arrays.asList(defaultValue.getClass().getEnumConstants()));
      // System.out.println("enum test: " + Enum.valueOf(defaultValue.getClass(),
      // "DASH1"));
    }

    void setBy(String s) {
      // note: value can never be null, or we'll need to store the Enum class
      // reference elsewhere
      // (e.g., in the Key -- better there anyway, where we could provide a generic
      // "values"
  ...
  /**
   * Handles CSS font-style value "italic" ("normal", or anything else, has no
   * effect as of yet)
   * Also handles CSS font-weight value of "bold" (anything else is ignored for
   * now)
  ...
  public class CSSFontFamilyProperty extends StringProperty {
    ...
    void setFromCSS(String cssKey, String value) {
      // no translation needed for now: just use the raw name -- if it's a preference
      // list tho, we'll need to handle it
      setBy(value);
    }
  }
  ...
  public class ColorProperty extends Property<java.awt.Color> {
    ...
    void set(Color newColor) {
      ...
      if (fixedAlpha < 0) {
        ...
        if (newColor != null && newColor.getAlpha() != fixedAlpha && newColor.getAlpha() != 0) {
          // out("COLOR VALUE: " + newColor + " " + ColorToString(newColor) + " alpha=" +
          // newColor.getAlpha());
          newColor = new Color((newColor.getRGB() & 0xFFFFFF) + (fixedAlpha << 24), true);
          // out("used fixed alpha " + fixedAlpha + " producing " + newColor + " alpha=" +
          // newColor.getAlpha()
    ...
    void take(Color c) {
      ...
      // super.take(c == null ? null : new Color(c.getRGB() + (fixedAlpha << 24),
      // true));
      // //super.take(c == null ? null : new Color(c.getRGB() + ((128 & 0xFF) << 24),
      // true));
    ...
    /**
     * @return a value between 0.0 and 1.0 representing brightness: the saturation %
     *         of the strongest channel
  ...
  // public static final Key KEY_StrokeStyle = new Key("stroke.style",
  // "border-style") { final Property getSlot(LWComponent c) { return null; } };
  ...
  /**
   * Aggregate font key, which represents the combination of it's three
   * sub-properties
   */
  ...
  private void rebuildStroke() {
     ...
     * // (Or was that just for Groups? No, those appear to be handling the full
     * bounds change.)
  ...
  private void rebuildFont() {
    // This so at least for now we have backward compat with the old font property
    // (esp. for tools & persistance)
  ...
  /**
   * set a generic property in the model -- users of this API need to ensure the
   * keys
   * they're using are unique with respect to any other potential users of this
   * API.
   * Setting a property value to null removes the property. This is basically
   * "cookies"
  ...
  // todo: support client data via arbitrary ENUM values (e.g., with EnumMap, and
  // maybe bits via an EnumSet)

  /**
   * convenience typing fetch when using a Class as a property key, that returns a
   * value casted to the class type
   */
   ...
   * @return object representing appropriate value, or null if none found (note:
   *         properties may be null also -- todo: fix)
   */

  public Object getPropertyValue(final Object key) {
    if (key instanceof Key) {
      // If getValue on the key was overriden, we may still need to trap an exception
      // here
      try {
  ...
  protected void setPropertyImpl(final Object key, final Object val, final Object context) {
    ...
    // }
      ...
      tufts.Util
          .printStackTrace("FYI: Unhandled Property key: " + key.getClass() + "[" + key + "] with value [" + val + "]");
    }
  }
  ...
  /**
   * Create a component with duplicate content & style. Does not duplicate any
   * links
   * to this component, and leaves it an unparented orphan. Leaving the parent
   * null
   ...
   *                    objects containing links that need to be reconnected at
   *                    the end of the duplicate.
   */
  ...
  /**
   * Provided for subclass impl's that need to support final members, which can
   * use a
   * pre-constructed empty object in their override of duplicate v.s. relying on
   * the default use
   * of newInstance via calls to super.duplicate. Technically, this also permits
   * any subclass of
   * instance of LWComponent to "duplicate" an instance of different subclass, and
   * only the
  ...
  protected <Ts extends LWComponent> Ts duplicateTo(Ts c, CopyContext cc) {
    ...
    c.mXMLRestoreUnderway = true; // todo: this flag really "initUnderway" -- need to double-check all our
                                  // semantics tho...
  ...
  protected final void ensureID(LWComponent c, boolean descendents) {
    ...
    // }
    // Not safe: tho shouldn't happen, if any parent<->child loops make their way
    // into the hierarchy, we can stack overflow here
  ...
  protected String getNextUniqueID() {
    if (getParent() == null) {
      // throw new IllegalStateException("LWComponent has null parent; needs a parent
      // instance subclassed from LWContainer that implements getNextUniqueID: " +
      // this);
      // if (DEBUG.PARENTING) tufts.Util.printStackTrace("getNextUniqueID: returning
      // null for current orphan " + this);
      if (DEBUG.PARENTING)
  ...
  /**
   * used for new node creation: todo -- get rid of this when we get rid of
   * getMetadataList()
   */
  public void takeAllDataValues(MetaMap map) {
    ...
    // ----------------------------------------------------------------------------------------
    // TODO: GET RID OF THIS DUPLICATION. This is done so that these can be UI
    // editable and so
    // RDFIndex will index these, making them searchable. Of course, only the COPIES
    // are
    // editable -- the originals stay the same. But adding these to the RDFIndex
    // directly from
    // the mDataMap would be trivial to add in RDFIndex.java, and do we really
    // need/want to be
  ...
  /**
   * @return true if the data-set data for this node represents a SINGLE VALUE
   *         from a field (e.g., one of an enumeration)
  ...
  /**
   * @return true if this is a single value data node of the given name from *any*
   *         schema
   */
  ...
  /**
   * @return true if this is a single value data node for the given Field in it's
   *         Schema
   */
  ...
  /**
   * @return null -- this is only needed for LWMap, but is implemented here to
   *         force
   *         the order of persistance based on the castor mapping, so schemas can
   *         persist
   *         before nodes in the LWMap, which overrides this to return schemas
   *         included in the
   *         map. If this was only declared in the mapping file as an LWMap
   *         persistance item,
   *         it would persist after all LWComponent mappings (LWMap subclasses
   *         LWComponent).
   */
  ...
  /**
   * Metadata List for use with RDF Index It is sufficient for the minimal RDF
   * functionality to
   * be able to retrieve this list from the LWComponent using this method and add
   * elements
   * directly to the list as needed. LWComponent may choose to create
   * notifications/modifcations
  ...
  /**
   * @return true if should not be drawn due to a currently applied filter, false
   *         if not.
   *         Note that FILTERING is diffrent than HIDING via a HideCause. A hidden
   *         node
   *         hides all of its children, but a filtered not does not (which can
   *         create
  ...
  public void setFiltered(boolean filtered) {
    // if (DEBUG.SEARCH&&DEBUG.TEST) Log.debug("setFiltered " + filtered + "; " +
    // this);
  ...
  protected boolean alive() {
    // "internal" objects should always report events (e.g., special styles, such as
    // data-styles)
    return parent != null
  ...
  /**
   * This should only be called once when added to the map, and on
   * deserializations
   */
    ...
    if (!mXMLRestoreUnderway) {
      ...
      notifyForce(LWKey.Created, new Undoable() {
        void undo() {
          ...
          // todo performance: force parents to always handle this so can skip creating
          // this event
  ...
  public void setLabelFormat(String s) {
    ...
    // if (alive()) notify("label.format", new Undoable(oldFormat) { void undo() {
    // setLabelFormat(oldFormat); }} );
  }
  ...
  private static boolean isDataFormatString(String s) {
    ...
    // however, it's okay to over-match, as replacements that can't be understood
    // are left as-is
  }

  /**
   * @param setDocument   -- we're called by TextBox after document edit with
   *                      setDocument=false,
   *                      so we don't attempt to re-update the TextBox, which has
   *                      just been
   *                      updated.
   *
   * @param allowFillData -- called with allowFillData=false if we don't
   *                      want to actually do a data-fill and just want to leave
   *                      the
   *                      label as the actual format (e.g. we just computed filled
   *                      data
   *                      and now want to set the label for real, or this is an
   *                      undo).
   */
  protected void setLabelImpl(String newLabel,
      final boolean setDocument,
      final boolean allowDataFill) {
    if (DEBUG.TEXT || DEBUG.DATA)
      out("setLabelImpl " + Util.tags(newLabel) + " setDoc=" + setDocument + " allowDataFill=" + allowDataFill);
  ...
  private String fillLabelFormat(final String fmt) {
    ...
    for (String s : parts) {
      ...
      try {
        ...
        Log.error(
            "exception processing replacement braces " + Util.tags(s) + " in format " + Util.tags(fmt) + " for " + this,
            t);
      }
  ...
  protected final int idStringToInt(String idStr) {
    ...
    // // special case for internal use objects, marked with '<' as initial
    // character
    // return -1;
    // }
   ...
   *         has no label set, generate a unique name for it, and if the label has
   *         any newlines
  ...
  public synchronized NodeFilter getNodeFilter() {
    // if the double-checked locking idiom was reliable in java, we'd use it here,
    // but
  ...
  /**
   * @deprecated -- return null if the node filter is empty, so we don't bother
   *             with entry in the save file
   */
  ...
  // TODO: resolve this with supportsProperty(LWKey.Label) (perhaps lose this
  // method)
  ...
  // that could have arbitrary properties added to it by plugged-in non-standard
  // tools
  ...
  /**
   * @return false: subclasses (e.g. containers), override to return true if
   *         allows children dragged in and out
   *         by a user.
   */
  ...
  /**
   * @Return true: subclasses (e.g. containers), override to return false if you
   *         never want this component
   *         reparented by users
   */
  ...
  /**
   * @return true: by default, all objects can be selected with other objects at
   *         the same time
   */
  ...
  /**
   * @return true if we allow a link to the target, and the target allows a link
   *         to us.
   *         Eventually we can use this to check ontology information.
   * @param target -- the target to check. If null, tells is if this component
   *               allows
  ...
  /**
   * @return null if we're in more than one visible pathway, or the LWPathway
   *         we're on if it's the only visible one
   */
  ...
  private void addPathwayRef(LWPathway p) {
    ...
    if (!mPathways.contains(p)) {
      mPathways.add(p);
      // todo: too late, UNDELETING flag already cleared (call is from pathway on
      // pathway undelete)
  ...
  /**
   * Layout this component and all children, if any. Normally this would only be
   * called on an
   * LWMap, but in some cases, any component might effectively be "at the top
   * level" while in an
   * intermediate state, such as components in a cut-buffer before they've been
   * pasted out to a
   * map.
   */
  ...
  public Color getRenderFillColor(DrawContext dc) {
    if (mFillColor.isTransparent()) {
      if (dc != null && dc.focal == this) {
        ...
        // System.out.println(" PARENT FILL: " + parent.getRenderFillColor(dc) + " " +
        // this);
  ...
  /**
   * @return null for SOLID (ordinal 0, the default, as for old save files), or
   *         otherwise, the ordinal of the style enum
   *         Castor will not bother to generate the attribute/element when it's
   *         value is null.
   */
  ...
  public void setXMLstrokeStyle(Integer ordinal) {
    // todo: have the Key class process enum's generically, caching the results of
    // Class<? extends Enum>.getEnumConstants()
   ...
   *
   * @return null if we are not a child of a layer, or the layer if it's our
   *         immediate parent
   */
  ...
  protected void setParent(LWContainer newParent) {
    ...
    if (parent == newParent) {
      ...
      // if (DEBUG.Enabled) Util.printStackTrace("redundant set-parent in " + this +
      // "; parent=" + newParent);
      return;
    }
  ...
  /**
   * for now (2007-11-30) just records sync source in case we want to use it
   * later,
  ...
  /**
   * set the given component as the style for this object, applying it's style
   * properties to us
   * Note: this will force the STYLE bit to be set on parentStyle if it already
   * isn't set
   */
  ...
  /**
   * @return Boolean.TRUE if this component is serving as an active style for
   *         other objects, null otherwise
   */
  ...
  /**
   * @return Boolean.TRUE if this component has marked as having the special
   *         "slide" style, null otherwise
   */
  ...
  /**
   * @return 0 by default
   *         the pick depth (in PickContext) must be >= what this returns for
   *         descdents of this component
   *         be picked (selected, etc). Mostly meaningful when an LWContainer
   *         subclass implements
   *         and returns something > 0, tho a single component could use this to
   *         become a "background" item.
   *         You can think of this as establishing a "wall" in the depth
   *         hierarchy, past which pick
   *         traversals will not descend unless given a high enough pickDepth to
   *         jump the wall.
   */
  ...
  // private static LWComponent ProxySlideComponent = new
  // LWComponent("<global-slide-proxy>");

  /**
   * return the component to be picked if we're picked: e.g., may return null if
   * you only want children picked, and not the parent
   */
  protected LWComponent defaultPick(PickContext pc) {
    ...
    if (pc.dropping != null && pc.dropping instanceof LWContainer && hasAncestor((LWComponent) pc.dropping))
      return null;
    // else if (isDrawingSlideIcon() && getMapSlideIconBounds().contains(pc.x,
    // pc.y)) {
  ...
  /**
   * If PickContext.dropping is a LWComponent, return parent (as we can't take
   * children),
  ...
  /**
   * @return true - A single component always "has content" -- subclasses override
   *         to provide varying semantics
   */
  ...
  /**
   * Although unsupported on LWComponents (must be an LWContainer subclass to
   * support children),
   * this method appears here for typing convenience and debug. If a non
   * LWContainer subclass
   * calls this, it's a no-op, and a diagnostic stack trace is dumped to the
   * console.
   */
  ...
  /**
   * @return the slides for drawing as slide icons in the current picking and
   *         drawing order
   */
  private Iterable<LWSlide> seenSlideIcons(DrawContext dc) {
    // if (mEntries == null || mEntries.size() == 0) {
    // // this is sort of overkill, as we shouldn't even be calling this if
    // hasEntries is false
  ...
  /**
   * @return a list, to be traversed in reverse order. If a new list needs to be
   *         constructed,
   *         it will dumped into stored, which will be returned. Otherwise, an
   *         internal list may be returned.
   */
  public List<LWComponent> getPickList(PickContext pc, List<LWComponent> stored) {
    ...
    if (pc.root != this && hasEntries() && (seenSlides = seenSlideIcons(pc.dc)) != Util.EmptyIterable) {
      // todo performance: see LWTraversal for comments: change impl to return a
      // ReverseListIterator, etc.a
      stored.clear();
      if (!isCollapsed())
  ...
  void addLinkRef(LWLink link) {
    ...
    if (mLinks.contains(link)) {
      // tufts.Util.printStackTrace("addLinkRef: " + this + " already contains " +
      // link);
      if (DEBUG.Enabled)
  ...
  public List<LWLink> getIncomingLinks() {
    // Note: it's inefficient to call getIncomingLinks() AND getOutgoingLinks()
    // because bi-directional
    // links will be returned by both and you'll double-process them. If you want
    // both incoming and
   ...
   *         Note that if this is called on an LWLink, it will only return objects
   *         linking to us,
  ...
  /**
   * given the set of links, which should be sub-set of our own links, return a
   * set containting the far endpoints
   */
  ...
  /**
   * @return all nodes that are *probable* clustering targets for this node.
   *         Currently,
  ...
  /**
   * @return all components directly connected to this one: for most components,
   *         this
   *         is just all the LWLink's that connect to us. For LWLinks, it's mainly
   *         it's endpoints,rg
   *         plus also any LWLink that may be directly connected to the link
   *         itself
   */
  ...
  /**
   * @return a list of every component connected to this one via links, including
   ...
   * @return a list of every component connected to this one via links, including
   *         the links themselves
   * @param bag - the collection to store the results in. Any component already in
   *            the bag will not
   *            have it's outbound links followed -- this provides inherent loop
   *            protection.
   *            Note that if this collection isn't a Set of some kind, components
   *            will appear in the bag more than once.
  ...
  /**
   * @return the union of the bounds of the current component, all connected
   *         links, and all far endpoints
   *         of those links.
   */
  ...
  /**
   * @return true of this component has any connections (links) to the given
   *         component.
   *         LWLink overrides to include it's endpoints in the definition of
   *         "connected" to.
   */
  ...
  /**
   * @return a collection of our ancestors. default impl returns a list with
  ...
  /**
   * @return the first ancestor, EXCLUDING this component (starting with the
   *         parent), that is of the given type, or null if none found
   */
  ...
  /**
   * @return the first ancestor, INCLUDING this component, that is of the given
  ...
  /**
   * @return by default, return the class object as returned by getClass().
   *         Subclasses can override to provide differentiation between runtime
   *         sub-types.
   *         E.g., a node class could return getClass() by default, but the
   *         constant string "textNode" for runtime instances that we
   *         want the tool code to treat is coming from a different class. Also
   *         note that supported property bits for
   *         all instances with a given type token should be the same.
   */
  public Object getTypeToken() {
    // todo: should really return null if we detect this is an instance of an
    // anonymous class
    // -- we don't want to be duplicating and using a style holder an instance of an
    // anon
    // glass that might be overriding god knows what and affecting property
    // setting/getting
    // Not that this will probably hurt anything: it'll never be referenced by a
    // VueTool,
  ...
  protected void setScale(double newScale) {
    ...
    // if (DEBUG.LAYOUT) tufts.Util.printClassTrace("tufts.vue", "setScale " +
    // scale);
    takeScale(newScale);
  ...
  /**
   * @return the scale value relative to it's parent. So for a 50% scale in it's
   *         parent,
   *         it just returns 0.5. E.g., this would mean if the parent was also
   *         scaled at 50%,
  ...
  /**
   * @return the on-map scale at 100% map scale (the concatentation of our scale
   *         plus all parent scales)
   */
  ...
  /**
   * @return true if this component is "owned" by the pathway -- e.g., a slide
   *         that only appears as an icon
   */
  ...
  /**
   * Translate this component within it's parent by the given amount -- quietly
   * w/out generating events
   */
  ...
  // {
  // // If this node exists in a scaled context, which means it's parent is scaled
  // or
  ...
  // // it's parent, not it's contents. So we need to beef up the translation
  // amount
  // // by the context scale so drags across the map will actually stay with the
  // // mouse. E.g., if this object exists in a parent scaled down 50%
  // (scale=0.5),
  // // to move this object 2 pixels to the right in absolute top-level map
  // // coordinates, we need to change it's internal location within it's parent
  // by 4
  // // pixels (2 / 0.5 = 4) to have that show up on the map (when itself
  // displayed
  ...
  /**
   * set the absolute map location -- meant to be overriden for special cases
   * (e.g., the special selection group)
   */
  ...
  /**
   * Set the location of this object within it's parent. E.g., if the parent is a
   * group or a slide,
   * setLocation(0,0) would move the component to the upper left corner of it's
   * parent. If the
   * parent is a map, (0,0) has no special meaning as the origin of Maps, while it
   * does exist,
  ...
  /**
   * Special setLocation to permit event notification during coordinate system
   * changes for objects not yet added to the map
   */
  protected void setLocation(float x, float y, LWComponent hearableEventSource, boolean issueMapLocationChangeCalls) {
    ...
    if (issueMapLocationChangeCalls) {
      ...
      // even have establishLocalCoordinates call us here with extra info...
      // (oldMapX/oldMapY)
      // or, we could implement the general setMapLocation and have
      // establishLocalCoords call that...

      // This code only works if we're moving within a single parent: no coordinate
      // system changes!
      ...
      // (e.g., the shape of curves isn't stored anywhere -- always needs to be
      // recomputed)
      // if (!linkNotificationDisabled)
      if (updatingLinks())
  ...
  protected void updateConnectedLinks(LWComponent movingSrc) {
    // if (!linkNotificationDisabled) // todo: if still end up using this feature,
    // need to pass this bit on down to children
    if (updatingLinks())
  ...
  /**
   * a notification to the component that it's absolute map location has changed
   * by the given absolute map dx / dy
   */
  ...
  // /** A notification to the component that it or some ancestor is about to
  // change parentage */
  ...
  /**
   * set component to this many pixels in size, quietly, with no event
   * notification
   */
  ...
  protected void setSizeImpl(float w, float h, boolean internal) {
    ...
    if (!internal && !javax.swing.SwingUtilities.isEventDispatchThread()) {
      // There was a reason this was important to do -- I think in some cases this
      // could
      // leave undoable state in the undo queue after sizes changes that were really
      // from
      // initializations, and we do NOT want to allow any undo back to a random
      // pre-init
      // size. [my guess is that this had/has to do with autosized] TODO 2012: BUG:
      // selection
  ...
  /*
   * set on screen visible component size to this many pixels in size -- used for
   * user set size from
  ...
  /*
   * getMapXXX methods are for values in absolute map positions and scales (needed
   * for VUE.RELATIVE_COORDS == true)
   * getScaledXXX methods are for VUE.RELATIVE_COORDS == false, tho I think we can
   * get rid of them? -- SMF
   *
   * "Map" values are absolute on-screen values that are true for any component in
   * a map rendered at 100% scale (the size & location)
  ...
  // // these two don't handle scale properly yet: need to adjust for parent
  // scales...
  ...
  protected void getLinkConnectionCenterRelativeTo(Point2D.Float point, LWContainer relative) {
    // if (relative == null) Util.printStackTrace("null relative for " + this + ": "
    // + relative);
    ...
    if (relative == this) {
      ...
      // if relative is null, just return available local data w/out accessing the
      // parent.
      ...
      if (this instanceof LWLink) {
        ...
        // works for connecting to inside a scaled context (e.g., a scaled down on-map
        // slide)
      ...
      // e.g., a combination of transformDown's then I guess transformUp's (would need
      // that),
      ...
      if (this instanceof LWLink) {
        // todo: consider getMapX/Y on LWLink override to return getParent().getMapX/Y
        // (need to check all calls tho...)
      ...
      if (DEBUG.Enabled) {
        if (relative != null && !hasAncestor(relative)) {
          // only if not the special invisible link endpoint, which has no parent (thus no
          // ancestors)
          if (getClass().getEnclosingClass() != LinkTool.LinkModeTool.class) {
            // String msg = "debug: " + this + " is computing link connetion center relative
            // to a non-ancestor: " + relative;
  ...
  /**
   * @return our center in our zero-based coordinate space: e.g., 1/2 our width.
   *         Links
   *         will compute differentely, as their zero-based coordinate space is
   *         their parent's space (same as local space)
   */
  ...
  // Oh tho -- I think in LWLink we need the mapX of US, plus the mapX of the
  // target
  // (if KEEP the ancestor code, implement generically so can pass in any value:
  // e.g, LWLink.mCurveCenterX)
  // -----------------------------------------------------------------------------
   ...
   * @return java.net.URI
   *
   *         returns a unique URI for a component. If component already has one it
   *         is returned else an new uri is created and returned.
   *         Would be nice if these were somehow persistenly unique via specified
   *         naming authorities. As they're currently
   *         not, this is kind of overkill for simple runtime unique indexing
   *         string lookups. These *are* persisted in
  ...
  public void setURIString(String URIString) {
    // if (isStyle) {
    // VUE.Log.warn("attempt to set URIString on a style: " + this + "; uriString="
    // + uri);
    // return;
    // }
    try {
  ...
  /**
   * @return our shape, full transformed into map coords and ultimate scale when
   *         drawn at 100% map zoom
   *         this is used for portal clipping, and will be imperfect for some
   *         scaled shapes, such as RountRect's
   *         This only works for raw shapes that are RectangularShapes -- other
   *         Shape types just return the bounding
   *         box in map coordinates (e.g., a link shape)
   */
  public RectangularShape getMapShape() {
    // Will not work for shapes like RoundRect when scaled -- e..g, corner scaling
    // will be off

    final Shape s = getZeroShape();
    // if (getMapScale() != 1f && s instanceof RectangularShape) { // todo: do if
    // any transform, not just scale
  ...
  /**
   * @return the raw shape of this object, not including any shape (the stroke is
   *         laid on top of the raw shape).
  ...
  /**
   * @return the raw, zero based, non-scaled shape; default impl returns same as
   *         getZeroBounds
   */
   ...
   *
   *         Altho the x/y of the rectangle will normally be 0,0 (suggesting we
   *         could just use
   *         a size object here), that's not always the case: a component who
   *         shares it's
   *         coordinate space with it's parent (such as a link) will usually have
   *         a non-zero
  ...
  /**
   * @return the PARENT based bounds -- this is the local component x,y
   *         width*scale,height*scale, where scale
   *         is any local scale this component has (not the total map scale: the
   *         scale that includes the scaling of all ancestors)
   */
  ...
  /**
   * @return the layout bounds -- this is the local bounds, plus an extra
   *         "hangoff" decorations that are not considered
   *         part of the formal bounds of the object. When the object is the
   *         focal, these items are not displayed.
   */
  ...
  /**
   * @return the PARENT based, non-scaled bounds including all extra-shape
   *         artifacts, such as a stroke
   */
  ...
  /**
   * @return getMapBounds() -- map-coord (absolute) bounds of the stroke shape
   *         (not including any stroke width)
   */
  ...
  /**
   * @return map-coord (absolute) bounds of the stroke shape (not including any
   *         stroke width)
   */
  ...
  /**
   * take the given map bounds, and add the scaled stroke width plus any extra if
   * given
   */
  ...
  /**
   * @return an AffineTransform that when applied to a graphics context, will have
   *         us drawing properly
   *         relative to this component, including any applicable scaling. So
   *         after this is applied,
   *         0,0 will draw in the upper left hand corner of the component
   */
  // create and recursively set a transform to get from the Map to this object's
  // coordinate space
  // note: structure is same in the different transform methods
  // TODO OPT: can cache this transform: if track all ancestor hierarcy, location
  // AND scale changes,
  ...
  /**
   * @return the transform that takes us from the given ancestor down to our local
   *         coordinate space/scale
   * @param ancestor -- the ancestor to get a transform relative to. If null, this
   *                 will return the
  ...
  /**
   * Called by model clients (e.g., MapViewer) for temporarily applying a special
   * transform to the
   * drawing and picking of a component without touching the underlying data model
   * (no persistent
   * changes to the component are made). The transform must be set to null to be
   * cleared.
  ...
  /**
   * transform relative to the child after already being transformed relative to
   * the parent
   */
  ...
  // // // When working on transformDownA, comment this code in, and comment out
  // transformDownG
  // // /** Must include overrides of all AffineTransform methods used in
  // transformDownA */
  ...
  // // /** transform relative to the child after already being transformed
  // relative to the parent */
  // // protected void transformDownG(final Graphics2D g) {
  // // GCAP.g = g; // not exactly thread-safe -- this temporary while we work on
  // this code (cut/paste duplicate when done)
  // // transformDownA(GCAP);
  // // }

  // /** transform relative to the child after already being transformed relative
  // to the parent */
  // protected void transformDownG(final Graphics2D a)
  // {
  // //-----------------------------------------------------------------------------
  ...
  /**
   * Will transform all the way from the the map down to the component, wherever
   * nested/scaled.
   * So drawing at 0,0 will draw in the upper left of the component.
   */
  public void transformZero(final Graphics2D g) {

    // todo: need a relative to parent transform only for cascading application
    // during drawing
   ...
   * @param zeroPoint the destination Point2D to place the resulting transformed
   *                  coordinate -- may be
   *                  the same object as mapPoint (it will be written over)
   * @return the transformed point (will be zeroPoint if transformed, mapPoint if
  ...
  public Point2D transformMapToZeroPoint(Point2D.Float mapPoint, Point2D.Float zeroPoint) {
    ...
    // if (!isZoomedFocus && scale == 1.0 && parent instanceof LWMap &&
    // !ROTATE_TEST) { // OPTIMIZATION
  ...
  /**
   * @param mapRect  -- incoming rectangle to transform to be relative to 0,0 of
   *                 this component
   * @param zeroRect -- result is placed here -- will be created if is null
   * @return zeroRect
   *
   *         E.g., if the incoming mapRect was from map coords 100,100->120,120,
   *         and this component was at 100,100,
   *         the resulting zeroRect in this case would be 0,0->20,20 (assuming no
   *         scale or rotation).
   *
   */
  // protected Rectangle2D transformMapToZeroRect(Rectangle2D mapRect, Rectangle2D
  // zeroRect)
  protected Rectangle2D transformMapToZeroRect(Rectangle2D mapRect) {
    // if (zeroRect == null)
    // zeroRect = (Rectangle2D) mapRect.clone(); // simpler than newInstace, tho we
    // won't need the data-copy in the end
  ...
  /**
   * This will take the given zeroRect rectangle in local coordinates, and
   * transform it
   ...
   * INTERSECTIONS always intersect based on map bounds, as opposed to contains,
   * which tests a local point.
   */
  public final boolean intersects(Rectangle2D mapRect) {
    return intersectsImpl(mapRect);
    // final boolean hit = intersectsImpl(rect);
    // //if (DEBUG.PAINT) System.out.println("INTERSECTS " + fmt(rect) + " " +
    // (hit?"YES":"NO ")
  ...
  /**
   * default impl intersects the render/paint bounds, including any borders (we
   * use this for draw clipping as well as selection)
   */
  protected boolean intersectsImpl(Rectangle2D mapRect) {
    ...
    if (DEBUG.PAINT && DEBUG.PICK)
      System.out
          .println("INTERSECTS " + fmt(mapRect) + " " + (hit ? "YES" : "NO ") + " for " + fmt(bounds) + " of " + this);
  ...
  /**
   * @return true if this component currently requires painting and intersects the
   *         master paint region
   */
  ...
  // /** if this component is selected and we're not printing, draw a selection
  // indicator */
  // // todo: drawing of selection should be handled by the MapViewer and/or the
  // currently
  ...
  /**
   * @return true if the given x/y (already transformed to our local coordinate
   *         space), is within our shape
   */
  ...
  /**
   * @return 0 means a hit, -1 a completely miss, > 0 means distance (squared), to
   *         be sorted out by caller
   */
  ...
  // local LWComponent contains/intersects code adjust for the local
  // transformation
  // themselves.
  ...
  // else if (true || mSlideIconBounds.x == Float.NaN) // need a
  // reshape/reshapeImpl trigger on move/resize to properly re-validate (wait: NaN
  // != NaN !)
  ...
  // Rectangle2D.Float slideIcon = (Rectangle2D.Float)
  // getSlideIconBounds().clone();
  // final float scale = getMapScaleF();
  ...
  /**
   * @return the local lower right hand corner of the component: for rectangular
   *         shapes, this is just [width,height]
  ...
  // // If shape is small, try and keep it from overlapping too much (esp the
  // label)
  // if (xoff < getWidth() / 2f)
  // xoff = getWidth() / 2f;
  ...
  public void drawLocal(DrawContext dc) {
    // this will cascade to all children when they draw, combining with their calls
    // to transformDown
    transformDownG(dc.g);

    final AffineTransform zeroTransform = DEBUG.BOXES ? dc.g.getTransform() : null;

    // if (dc.focal == this || dc.isFocused()) // prevents slide icons from
    // appearing in portals
    if (dc.focal == this)
      drawZero(dc);
    else
   ...
   * drawing or redrawing a single component at it's proper map location. The
   * passed
   * in DrawContext gc is expected to be transformed for drawing the top-level map
   * (minimally transformed). If you are going to use the passed in DrawContext
   * after
   * this call for other map drawing operations, be sure to pass in dc.create()
   * from
   * the caller, as this call will leave the DrwaContext it in a generally
   * undefined state
  ...
  public void draw(DrawContext dc) {
    ...
    // dc.setClipOptimized(false); // ensure all children draw even if not inside
    // clip
  ...
  /**
   * fit and center us into the total clip bounds of the given dc -- border gap
   * pixels will multiplied by final scale value
   */
   ...
   *
   * @param borderGap: if positive, a fixed amount of pixels, if NEGATIVE, the %
   *                   of viewport size to leave as a margin
   */
  ...
  // /** If there's a pathway entry we want to be showing, return it, otherwise,
  // null */
  ...
  // path = mPathways.get(0); // show the first pathway it's in if it's not in the
  // active pathway
  // else
  // path = null;
  // }
  ...
  // TODO: need to do this as part of layout: need to trigger layout if any
  // pathway
  // visibility or membership changes. Currently, the paint bounds falls behind as
  // farthestVisibleCorner doesn't update till draw time... We special case a call
  // to
  // this during the init (restore) layout, so at least auto-fit at startup works,
  // but
  // if we ever hava a viewer that's implementing a constant auto-fit feature, it
  // will
  ...
  private void layoutSlideIcons(DrawContext dc) {
    ...
    // // if presenting, let the position the active pathway slide as the last slide
    // in the stack
    ...
    for (LWPathway.Entry e : mEntries) {
      if (e.hasVisibleSlide()) {
        ...
        // slide.setLocation(xoff, yoff); // No good: if slide's parent moves, the slide
        // moves..
        slide.takeLocation(xoff, yoff);
  ...
  private void drawSlideIcon(final DrawContext dc, final LWSlide slide) {
    ...
    // dc.g.setClip(curClip); // TODO: this is clearing the underlying clip and
    // allowing the border to draw over the scroll bars, etc!
   ...
   *
   * // A hack so that when LWLinks (hasAbsoluteMapLocation) pop to map drawing,
   * they
   * // don't pop up beyond this point.
   * //dc.mapTransform = dc.g.getTransform();
   *
   * //dc.g.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER,
   * 0.9f));
   ...
   * if (entry != null) {
   * // if we had an entry, but it was a map-view slide, do something to make it
   * look slide-like
  ...
  /**
   * default impl: does nothing -- meant to be overriden -- meant to potentially
   * cache all children
   */
  ...
  /**
   * @param eventMask is a string constant (from LWKey) or an array of such. If
   *                  one
   *                  of these non-null values, only events matching those keys
   *                  will be delievered
   */
  ...
  /**
   * convenince method for remove a (possible) old listener, and attaching a
   * (possible) new listener
   */
  ...
  protected synchronized void notifyLWCListeners(LWCEvent e) {
    ...
    // if (e.key == LWKey.UserActionCompleted || e.key == LWKey.Location && e.source
    // == this) {
    ...
    // ; // ignore events from non-embedded style objects (e.g., EditorManager
    // constructs)
    // else
    // mChangeSupport.notifyListeners(this, e);
  ...
  /**
   * Copy the value for the given key either back to our sync source, or to our
   * sync clients
   */
  ...
  protected void updateStyleWatchers(Key key, LWCEvent e) {
    ...
    for (LWComponent dest : findPotentialStyleWatchers()) {
      ...
      if (dest.mParentStyle == this && dest.supportsProperty(key) && dest != this) {
        // Only copy over the style value if was previously set to our existing style
        // value
        try {
  ...
  // /** This generates an event with NO COMPONENT IN IT: we just want access to
  // the model hierarchy at
  ...
  protected void notify(String what, Object oldValue) {
    // // TODO PERFORMANCE: have EVENT_SILENT always be true for initializing nodes
    // -- this
    // // should speed things up when creating hundreds/thousands(!) of nodes,
    // including
    // // during map restores -- we can skip creating an event that will never be
    // delievered
  ...
  /**
   * same as notify(String, Object), but will do notification even if the
   * LWComponent isn't "alive" yet
   */
  ...
  /**
   * Delete this single component: equivalent to a user-delete action. This
   * component
   * will be removed from it's parent, and disconnected from all relationships in
   * the
   * model.
   */
  public void delete() {
  ...
  protected void removeFromModel() {
    ...
    // throw new IllegalStateException(this + ": attempt to delete already
    // deleted");
    setFlag(Flag.DELETING);
    notify(LWKey.Deleting);
    prepareToRemoveFromModel();
    removeAllLWCListeners();
    disconnectFromLinks(); // if any of the links themseleves are being deleted, we don't actually need to
                           // disconnect
  ...
  protected void restoreToModel() {
    ...
    // we need this only in case node-icon preferences have changed since this
    // object left the
    // model -- otherwise nothing could have happened to this component to change
    // it. Any
    // possible size/location events that could happen as a result will be ignored
    // as this
  ...
  private void disconnectFromLinks() {
    // iterate through copy of the list, as it may be modified concurrently during
    // removals
    if (mLinks != null) {
  ...
  protected boolean selectedOrParent() {
    ...
    // return parent == null ? isSelected() : (parent.selectedOrParent() |
    // isSelected());
  }
  ...
  public static final Key KEY_State = new Key<LWComponent, Integer>("state") {
    ...
    // arch: would be nice if all the stuff for dealing with bitfields was handled
    // in a single
  ...
  public void setState(State s, boolean on) {
    ...
    // (2) LWLink internal head/tail prune states also need to be synced -- would
    // need to make these
  ...
  /**
   * @return true if ths component is going to be painting itself (independent of
   *         any children may do so)
   */
  ...
  /**
   * @return true if this node may have any drawing to do: (e.g., itself or
   *         children)
   *         Note that a return of true does not guarantee that we will draw
   *         anything,
  ...
  public boolean handleDoubleClick(MapMouseEvent e) {
    ...
    {
      // } else if (this instanceof LWSlide || this instanceof LWGroup || this
      // instanceof LWPortal)
      // MapViewer "null remote focal" code would need fixing to enable selection if a
      // portal is the focal
      // (the selected objects are not children of the focal, so they don't look like
  ...
  /**
   * any XML tag found in a save file that does not match a mapping in from the
   * current mapping file shows
   * up here -- they appear to always be instances of
  ...
  public void XML_addNotify(Object context, String name, Object parent) {
    if (DEBUG.XML && DEBUG.META)
      tufts.Util.printClassTrace("tufts.vue", "XML_addNotify; name=" + name
          + "\n\tparent: " + parent
          + "\n\t child: " + this
          + "\n");
  ...
  public void XML_completed(Object context) {
    ...
    // layout(); need to wait till scale values are all set: so the LWMap needs to
    // trigger this
  }
   ...
   * @param maxSize -- if non-null, the max width/height of the produced image
   *                (may be smaller)
   * @param zoom    -- a zoom for the map size in producing the image (currently
   *                ignored if maxSize is provided)
   */
  ...
  /**
   * @return the map bounds to use for rendering when generating an image of this
   *         LWComponent
   */
  ...
  /**
   * Create a new buffered image, of max dimension maxSize, and render the
   * LWComponent
   * (and all it's children), to it using the given alpha.
   *
   * @param alpha       0.0 (invisible) to 1.0 (no alpha)
   * @param maxSize     max dimensions for image. May be null. Image may be
   *                    smaller than maxSize.
   * @param fillColor   -- if non-null, will be rendered as background for image.
   *                    If null, presume alpha 0 fill.
   * @param zoomRequest -- desired zoom; ignored if maxSize is non-null
   *                    also set, background fill will have transparency of
   *                    alpha^3 to enhance contrast.
   */

  // Note: as of Mac OS X 10.4.10 (Intel), when a java drag source declares it can
  // generate an image (as we do when we Apple-drag something), if you drop it on
  // the
  // desktop, it will create a special mac "picture clipping", which is some kind
  // of
  ...
  // opening it up it shows entirely as a blank space (I think if the image starts
  // to
  // get "very large"), tho the data is actually there -- if you drag the picture
  // clipping into an Apple Mail message, it shows up again (and if you dragged
  // from
  // VUE to Apple Mail in the first place, it also works fine). Note that AFTER
  // dragging into Apple Mail, you can THEN double-click the attachment, and it
  // will
  // open it up in Preview as a .tiff file (Apple Mail appears to be converting
  // the
  // .pictClipping to tiff). Note that uncompressed TIFF isn't exactly a friendly
  // mail attachment format as it's huge. But once you open the image in Preview,
  // you
  // have the option of saving it / exporting it as a jpeg, and you can even
  // adjust
  ...
  public BufferedImage createImage(double alpha, Dimension maxSize, Color fillColor, double zoomRequest) {
    ...
    // final boolean fillHasAlpha = (fillColor != null && fillColor.getAlpha() !=
    // 255);
    ...
    // + "\n\t usedZoom: " + usedZoom
    // + "\n\t type: " + (imageType == BufferedImage.TYPE_INT_RGB ? "OPAQUE" :
    // "TRANSPARENT")
    // + TERM_CLEAR);
  ...
  /**
   * Useful for drawing drag images into an existing graphics buffer, or drawing
   * exportable images.
   *
  ...
  public void drawImage(Graphics2D g, double alpha, Dimension maxSize, Color fillColor, double zoomRequest) {
    ...
    if (fillColor != null) {
      ...
      // fillColor = new Color(c.getRed(), c.getGreen(), c.getBlue(), (int)
      // (alpha*alpha*255+0.5));
      // }
      if (alpha != OPAQUE)
      ...
      // we didn't have a fill, but we have an alpha: make sure any cached data is
      // cleared
      // todo?: if fill is null, we need to clear as well -- it means we have implied
      // alpha on any non-drawn bits
      // TODO: if this is a selection drag, we usually want to fill with the map color
      // (or ideally, the color
    ...
    if (DEBUG.IMAGE && DEBUG.META) {
      ...
      dc.g.setColor(Color.red);
      dc.g.fillRect(-Short.MAX_VALUE / 2, -Short.MAX_VALUE / 2, // larger values than this can blow out internal GC code
                                                                // and we get nothing
  ...
  protected void out(String s) {
    ...
    // if (DEBUG.Enabled) LWLog.debug(String.format("%6s[%-12.12s] %s", typeName,
    // getDisplayLabel(), s));
  ...
  public String toString() {
    ...
    // if (mHideBits != 0) s += " " + getDescriptionOfSetBits(HideCause.class,
    // mHideBits);
