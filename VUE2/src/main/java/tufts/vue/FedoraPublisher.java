...
public class FedoraPublisher {
  ...
  public static void uploadMapAll(edu.tufts.vue.dsm.DataSource ds, LWMap map) throws Exception {
    ...
    for (LWComponent component : cloneMap.getAllDescendents(LWComponent.ChildKind.PROPER)) {
      if (component.hasResource() && (component instanceof LWNode || component instanceof LWLink)
        ...
        if (resource.isLocalFile()) {
          ...
          String ingestUrl = HTTP + "://" + properties.getProperty("fedora22Address") + ":"
              + properties.getProperty("fedora22Port") + FEDORA_URL_PATH + "get/" + pid + "/" + RESOURCE_DS;
  ...
  private static void addObjectToRepository(edu.tufts.vue.dsm.DataSource ds, String cModel, File file, LWComponent comp,
      LWMap map) throws Exception {
    Properties properties = ds.getConfiguration();
    ...
    FedoraClient fc = new FedoraClient(HTTPS + "://" + properties.getProperty("fedora22Address") + ":"
        + properties.getProperty("fedora22SecurePort") + FEDORA_URL_PATH, properties.getProperty("fedora22UserName"),
  ...
  private static void addObject(FedoraClient fc, Properties p, String cModel, File file, LWComponent comp, LWMap map)
      throws Exception {
  ...
  private static void modifyObject(FedoraClient fc, Properties p, String cModel, File file, LWComponent comp, LWMap map)
      throws Exception {
    String pid = getFedoraPid(comp);
    ...
    fc.getAPIM().modifyDatastreamByValue(pid, DC_DS, null, DC_LABEL, XML_MIME_TYPE, DC_URL, dcXML.getBytes(), null,
        null, COMMENT, true);
    // modifying rels-ext for non VUE_CM
    if (!cModel.equals(VUE_CM)) {
      fc.getAPIM().modifyDatastreamByValue(pid, RELS_DS, null, RELS_LABEL, XML_MIME_TYPE, RELS_URL,
          getRDFDescriptionForLWComponent(comp, map).getBytes(), null, null, COMMENT, true);
    }
    if (cModel.equals(REMOTE_CM)) {
      fc.getAPIM().modifyDatastreamByReference(pid, dsName, null, comp.getLabel(), mimeType, VUE_FORMAT_URL,
          comp.getResource().getSpec(), null, null, COMMENT, true);
    } else {
      dsName = file.getName();
      Uploader uploader = new Uploader(HTTPS, p.getProperty("fedora22Address"),
          Integer.parseInt(p.getProperty("fedora22SecurePort")), p.getProperty("fedora22UserName"),
          p.getProperty("fedora22Password"));
      String uploadId = uploader.upload(file);
      fc.getAPIM().modifyDatastreamByReference(pid, dsName, null, comp.getLabel(), mimeType, VUE_FORMAT_URL, uploadId,
          null, null, COMMENT, true);

    }
  }

  private static String getDigitalObjectXML(Properties p, LWComponent comp, LWMap map, String cModel, File file)
      throws Exception {
    String pid = getFedoraPid(comp);
  ...
  private static String getOjectDSXML(Properties p, LWComponent comp, String cModel, File file) throws Exception {
    ...
    if (cModel.equals(VUE_CM)) {
      dsName = file.getName();
      mimeType = VUE_MIME_TYPE;
      r += getDSXML("R", MAP_DS, dateString, comp.getLabel(),
          "http://local.fedora.server/fedora/get/" + pid + "/" + dsName, "URL", mimeType);
    }
    if (!cModel.equals(REMOTE_CM)) {
      // dsName = URLEncoder.encode(file.getName(), ENCODING);
      dsName = file.getName();
      // Uploader uploader = new Uploader(HTTPS,
      // p.getProperty("fedora22Address"),Integer.parseInt(p.getProperty("fedora22SecurePort")),p.getProperty("fedora22UserName"),
      // edu.tufts.vue.util.Encryption.decrypt(p.getProperty("fedora22Password")));
      Uploader uploader = new Uploader(HTTPS, p.getProperty("fedora22Address"),
          Integer.parseInt(p.getProperty("fedora22SecurePort")), p.getProperty("fedora22UserName"),
          p.getProperty("fedora22Password"));
      uploadId = uploader.upload(file);
      r += getDSXML("R", RESOURCE_DS, dateString, comp.getLabel(),
          "http://local.fedora.server/fedora/get/" + pid + "/" + dsName, "URL", mimeType);
    } else {
      controlGroup = "E";
  ...
  public static String getDSXML(String controlGroup, String dsName, String dateString, String label, String uploadId,
      String contentLocationType, String mimeType) {
    StringBuilder xml = new StringBuilder();
    xml.append("<foxml:datastream CONTROL_GROUP=\"" + controlGroup + "\" ID=\"" + dsName
        + "\" STATE=\"A\" VERSIONABLE=\"true\">\n");
    xml.append(
        "<foxml:datastreamVersion CREATED=\"" + dateString + "\" FORMAT_URI=\"http://vue.tufts.edu/docs/vueformat/\"");
  ...
  private static String getDCXML(LWComponent comp) {
    ...
    xml.append("<foxml:datastreamVersion CREATED=\"" + dateString
  ...
  private static String getDC(LWComponent c, String title, String identifier) {
    ...
    // System.out.println("getDC: LWComponent: "+c+ " metadata elements: "+
    // c.getMetadataList().getMetadata().size());
    ...
    for (VueMetadataElement element : c.getMetadataList().getMetadata()) {
      // System.out.println("Publishing Metadata: key:"+element.getKey()+ " value: "+
      // element.getValue()+" for "+c.getLabel());
  ...
  private static String getRDFDescriptionForLWComponent(LWComponent comp, LWMap map) {
    ...
    for (LWComponent c : map.getAllDescendents(LWComponent.ChildKind.PROPER)) {
      for (VueMetadataElement element : c.getMetadataList().getMetadata()) {
        if (DEBUG.RDF)
          System.out.println(
              "METADATA: " + element.getValue() + " key:  " + element.getKey() + " LWComponent: " + comp.getLabel());
        if (element.getKey().equals(ONT_TYPE_METADATA) && element.getValue().startsWith(FEDORA_ONTOLOGY)
            && c instanceof LWLink) {
          LWLink link = (LWLink) c;
          LWComponent head = link.getHead();
          if (getFedoraPid(head).equals(getFedoraPid(comp))) {
            if (DEBUG.RDF)
              System.out.println("METADATA MATCH COMPONENT: " + element.getValue() + " key:  " + element.getKey()
                  + " LWComponent: " + comp.getLabel() + " Link:" + link);
            LWComponent tail = link.getTail();
            rdfDescription += "<rel:" + getFedoraOntologyTerm(element.getValue()) + " rdf:resource=\"info:fedora/"
                + getFedoraPid(tail) + "\" />\n";
          }
        }
      }
